...
class Api_v13 extends CI_Controller {
	...
	$this->sendsms($mobile,$msg);
       ...
       if(isset($data->checkon->linked)){
        ...
        $msg="Dear User, Your mobile verification code for login in MID App is $otp. Please Use this code to verify your mobile No.";
         ...
         if($mobile=="1234567890" || $mobile=="1234567888" || $mobile=="0888088808"){
             $otp="8888";
         }
         $data=array(
           'otp'=>$otp,
 ...
 function addEmpLeave(){
  ...
  function addOfflineAttendance(){
    ...
    //   }
      ...
      $sendRes = array('msg'=>$msg,'status'=>$status,'offline'=>$newOffline,'company_data'=>$cmp_array,'holiday'=>$holidays,'group'=>$groups,'leaves'=>$leaves,'logout'=>'0','sections'=>$sections,'user_section'=>$check['section'],'options'=>$options,'user_images'=>$users_images,'task'=>$tasks);
      ...
      if(!empty($check['id']) && $check['user_group']==1 && $check['prime_att']==1){
        ...
        if(!empty($users_data)){
          ...
          foreach($users_data as $user){
            ...
            $offline_at = $this->app->getUserAttendanceByCompany($user->user_id,$check['id'],$start_time,$end_time);
            $leaves = $this->app->getEmpLeaves($user->user_id);
          ...
          echo $response= json_encode(array('checkon'=>$new_array,'groups'=>$groups,'holidays'=>$holidays,'user_request'=>$users_data,'prime'=>'1','sections'=>$sections,'tasks'=>$tasks));
        }else{
          $res=array('msg'=>'No Data Foud');
          echo $response= json_encode(array('checkon'=>$res));
        }
  ...
  function getCompanyTask(){
    ...
    if(key($data)=="checkon"){
      ...
      if(!empty($check['id']) && $check['user_group']==1 && $check['prime_att']==1){
        $tasks = $this->app->getCompanyTask($check['id']);
        echo $response= json_encode(array('checkon'=>$tasks));
      }else{
          $res=array('msg'=>'Currently Inactive','prime'=>'0');
          echo $response= json_encode(array('checkon'=>$res,'prime'=>'0'));
      }
