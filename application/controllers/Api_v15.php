...
class Api_v15 extends CI_Controller {
 function __construct(){
       parent::__construct();
   $this->load->database();
   $this->load->model('Api_Model_v9','app');
  ...
  function updateEmpOptions(){
   ...
   if(key($data)=="checkon"){
     ...
     if(!empty($check['id']) && $check['user_group']==1){
       ...
       if($dt){
         $res = $this->app->updateCmpOptions($check['id'],$data->checkon->auto_gps,$data->checkon->qr,$data->checkon->gps,$data->checkon->field_duty,$data->checkon->four_layer_security,$data->checkon->face,$data->checkon->selfie_with_gps,$data->checkon->selfie_with_field_duty,$data->checkon->gps_tracking);
       }else{
         $options=array(
           'bid'=>$check['id'],
           'qr'=>$data->checkon->qr,
       ...
       $res = $this->app->updateEmpOptions($check['id'],$data->checkon->auto_gps,$data->checkon->qr,$data->checkon->gps,$data->checkon->field_duty,$data->checkon->four_layer_security,$data->checkon->face,$data->checkon->selfie_with_gps,$data->checkon->selfie_with_field_duty,$data->checkon->gps_tracking);
       if($res){
    ...
    if(key($data)=="checkon"){
      ...
      if(!empty($check['id']) && $check['user_group']==1 && $check['prime_att']==1){
        ...
        if(!empty($users_data)){
          ...
          echo $response= json_encode(array('checkon'=>$new_array,'groups'=>$groups,'holidays'=>$holidays,'user_request'=>$users_data,'prime'=>'1','sections'=>$sections,'tasks'=>$tasks,'rules'=>$rules));
        }else{
          $res=array('msg'=>'No Data Foud');
          echo $response= json_encode(array('checkon'=>$res));
        }
