...
class Api_v2 extends CI_Controller {
 function __construct(){
       parent::__construct();
   $this->load->database();
   $this->load->model('Api_Model_v2','app');
 ...
 function addUserAttendance(){
   ...
   if(key($data)=="checkon"){
     ...
     if(!empty($check['id']) && $check['user_group']==2){
       ...
       if( !empty($check['company']) && $check['company']==$loginid){
         if($user_status['user_status']=="1"){
           $data = array(
...
function getCompanyUsersByStatus(){
 ...
 function getEmpProfile(){
   ...
   if(key($data)=="checkon"){
     $check=$this->app->checkMobile($data->checkon->mobile);
     if(!empty($check['id']) && $check['user_group']==1){
       $emp = $this->app->getEmpProfile($data->checkon->uid);
          ...
          if($num_month>31){
           ...
           for($d=0; $d<$num_month;$d++){
             ...
             if($user->doj!="" && $new_start_time>=$user->doj){
                 $user_at = $this->app->getUserAttendanceByDate($new_start_time,$new_end_time,$user->user_id,$check['id'],1);
                 $data = array();
 ...
 function userLeft(){
   ...
   if(key($data)=="checkon"){
     $check=$this->app->checkMobile($data->checkon->mobile);
     if(!empty($check['id']) && $check['user_group']==1){
       $res = $this->app->userLeft($data->checkon->userId,$check['id']);
         if($res == 1){
           $sendRes = array('msg'=>'Left Successfully','status'=>'1');
         }else{
           $sendRes = array('msg'=>'Failed to left','status'=>'0');
         }
         echo $response= json_encode(array('checkon'=>$sendRes));
     }
   }
   ...
   if(key($data)=="checkon"){
     ...
     if(!empty($check['id']) && $check['user_group']==1){
       ...
       if(!empty($newQr)){
         ...
         if($check['id']==$userMobile['company']){
           if($user_status['user_status']=="1"){
               $mode = "in";
 ...
 function managerNewQrAttendance(){
   ...
   if(key($data)=="checkon"){
     ...
     if(!empty($check['id']) && $check['user_group']==2 && $check['manager']==1){
       ...
       if(!empty($newQr)){
         $loginid=$newQr['login_id'];
         $user_status = $this->app->userCmpStatus($loginid,$check['company']);
         $userMobile=$this->app->checkMobile($newQr['mobile']);
         if($check['company']==$userMobile['company']){
           if($user_status['user_status']=="1"){
               $mode = "in";
           $start_time = strtotime(date("d-m-Y 00:00:00"));
           $end_time = strtotime(date("d-m-Y 23:59:59"));
           $res_mode = $this->app->getUserAttendanceByDate($start_time,$end_time,$loginid,$check['company'],1);
