...
class Api_v4 extends CI_Controller {
 function __construct(){
       parent::__construct();
   $this->load->database();
   $this->load->model('Api_Model_v4','app');
 ...
 function addEmpLeave(){
  ...
  function addOfflineAttendance(){
    ...
    //   }
      ...
      $sendRes = array('msg'=>$msg,'status'=>$status,'offline'=>$offline,'company_data'=>$cmp_array,'holiday'=>$holidays,'group'=>$groups,'leaves'=>$leaves,'logout'=>'0','sections'=>$sections,'user_section'=>$check['section']);
    ...
    if(key($data)=="checkon"){
      ...
      if(!empty($check['id']) && $check['user_group']==1){
        ...
        if(!empty($users_data)){
          ...
          foreach($users_data as $user){
              if(($user->doj!="" || $start_time>=$user->doj) && ($user->left_date=="" || $start_time<$user->left_date)){
                  $user_at = $this->app->getUserAttendanceReportByDate($start_time,$end_time,$user->user_id,$check['id'],0);
                $data = array();
  ...
  function updateEmpOptions(){
   ...
   if(key($data)=="checkon"){
     $check=$this->app->checkMobile($data->checkon->mobile);
     if(!empty($check['id']) && $check['user_group']==1){
       $cmpOp = $this->app->getCmpOptions($check['id']);
       if(!empty($cmpOp)){
         $res = $this->app->updateCmpOptions($check['id'],$data->checkon->auto_gps);
       }else{
         $options=array(
           'bid'=>$check['id'],
       ...
       $res = $this->app->updateEmpOptions($check['id'],$data->checkon->auto_gps);
       if($res){
