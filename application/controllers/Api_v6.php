...
class Api_v6 extends CI_Controller {
 function __construct(){
       parent::__construct();
   $this->load->database();
   $this->load->model('Api_Model_v6','app');
...
function getCompanyUsersByStatus(){
 ...
 function updateEmpProfile(){
   ...
   if(key($data)=="checkon"){
     $check=$this->app->checkMobile($data->checkon->mobile);
     if(!empty($check['id']) && $check['user_group']==1){
      $doj = $this->app->updateDoj($data->checkon->uid,$check['id'],strtotime($data->checkon->doj),$data->checkon->rule_id,$data->checkon->qr,$data->checkon->gps,$data->checkon->face,$data->checkon->colleague,$data->checkon->autoGps,$data->checkon->gpsTracking,$data->checkon->field_duty,$data->checkon->four_layer_security);
  ...
  function updateEmpOptions(){
   ...
   if(key($data)=="checkon"){
     ...
     if(!empty($check['id']) && $check['user_group']==1){
       ...
       if($dt){
         $res = $this->app->updateCmpOptions($check['id'],$data->checkon->auto_gps,$data->checkon->qr,$data->checkon->gps,$data->checkon->field_duty,$data->checkon->four_layer_security);
       }else{
         $options=array(
           'bid'=>$check['id'],
           'qr'=>$data->checkon->qr,
       ...
       $res = $this->app->updateEmpOptions($check['id'],$data->checkon->auto_gps,$data->checkon->qr,$data->checkon->gps,$data->checkon->field_duty,$data->checkon->four_layer_security);
       if($res){
