        ...
        <div class="row mb-2">
          ...
          <div class="col-sm-12">
            <ol class="breadcrumb float-sm-right">
              ...
              <li class="breadcrumb-item active">Short Leave Report</li>
            </ol>
          </div>
        </div>
      </div><!-- /.container-fluid -->
          ...
          <div class="col-md-12">
            <div class="card card-primary">
              ...
              <table id="example1" class="table table-bordered table-striped">
                  ...
                  <tbody>
                      <?php
                      $res=$this->web->getSLeaveByBusinessId($bid);
                      if($this->session->userdata()['type']=='P'){
                        if($role[0]->type!=1){
                     ...
                     &nbsp;&nbsp;
                     <button class="btn btn-danger" onclick="reject('<?php echo $res->id;?>' ,' <?php echo $uid ;?>',' <?php echo $fromdate ;?>')">Reject </button>
					            <?php	} elseif($res->status==3){ echo "Rejected"; ?>
						          &nbsp;&nbsp;
                     <button class="btn btn-success" onclick="aprove2('<?php echo $id ;?>' ,' <?php echo $uid ;?>',' <?php echo $fromdate ;?>')">Aprove</button>
                    <?php     } ?>
                        </td>
                    <!--    
...
<script type="text/javascript">
  ...
  function aprove(id,uid,fromdate){
    $.ajax({
      type: "POST",
      url: "User/aproveSUser",
      data: {id,uid,fromdate},
    success: function(result){
		 alert('Short Leave Aproved ');
		// $("#status").html("Aproved");
  ...
  function aprove2(id,uid,fromdate){
    $.ajax({
      type: "POST",
      url: "User/aproveSUser",
      data: {id,uid,fromdate},
