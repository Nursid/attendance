...
class Api_f4 extends CI_Controller {
 function __construct(){
       parent::__construct();
   $this->load->database();
   $this->load->model('Api_Model_f4','app');
...
public function login_old(){
   ...
   if(key($data)=="checkon"){
	...
	$this->sendsms($mobile,$msg);
       ...
       if($checkData > 0){
         $response=array('msg'=>'OTP Send Successfully!','mobile'=>$mobile,'user_group'=>2);
 ...
 public function verifyOtp(){
   ...
   if(key($data)=="checkon"){
     ...
     if(!empty($data)){
        ...
        $userCmp = $this->app->getUserCompany($data['id']);
        if(!empty($userCmp['business_id'])){
            $data['hostel']=$userCmp['hostel'];
        }else{
            $data['hostel']="0";
        }
        
        foreach($linked as $key=>$link){
            $linkCmp = $this->app->getUserCompany($link->id);
            if(!empty($linkCmp['business_id'])){
                $linked[$key]->hostel=$linkCmp['hostel'];
            }else{
                $linked[$key]->hostel="0";
            }
        }
     ...
     if(!empty($check['id']) && $check['user_group']==2){
         if($check['device_id']==$data->checkon->device_id){
           ...
           if(isset($userCmp) && ($userCmp['left_date']=="" || $userCmp['left_date']>time())){
             ...
             $field_res = $this->app->getUserOdAttendanceByDate($startTimestamp,$endTimestamp,$check['id'],$userCmp['business_id']);
             $prev = 0;
             $next = 0;
             ...
             rsort($month_array);
                   echo $response= json_encode(array('checkon'=>$month_array,'company_data'=>$cmp_array));
           }else{
...
function isEmply(){
    ...
    if (key($data) == "checkon") {
        ...
        if (!empty($check['id']) && $check['user_group'] == 2) {
            $userCmp = $this->app->getUserCompany($check['id']);
            $TeacherCmp = $this->app->getTeacherCompany($check['id']);

            if(!empty($TeacherCmp)){
                $res = array('msg' => 'Teacher Found', 'status' => '2');
                echo json_encode(array('checkon' => $res));
            }elseif(!empty($userCmp)){
                $res = array('msg' => 'Employee Found', 'status' => '1');
                echo json_encode(array('checkon' => $res)); 
            } else {
                $res = array('msg' => 'Please Add Company', 'status' => '0');
                echo json_encode(array('checkon' => $res));
   ...
   if(key($data)=="checkon"){
     ...
     if(!empty($check['id']) && $check['user_group']==2){
         if($check['device_id']==$data->checkon->device_id){
           ...
           if ( (isset($TeacherCmp) )){
               $company_id = $TeacherCmp[0]->bid ; 
             $company = $this->app->getbussnames($company_id);
            // $user_status = $this->app->userCmpStatus($check['id'],$company_id);
    
             $startMonth = $this->app->attStartMonth($check['id']);
    
             $holidays = $this->app->getHoliday($company_id);
    
             $doj = $this->app->getDojt($check['id'],$company_id);
            ...
            rsort($month_array);
             ...
             $cmp_array = array('company_name'=>"",'company_image'=>"",'status'=>'0','preMonth'=>0,'nextMonth'=>0,'month'=>'0');
             $res = array('msg'=>'Please Add Company','status'=>'0');
             echo $response= json_encode(array('checkon'=>$res,'company_data'=>$cmp_array));
           }
     ...
     if(!empty($check['id']) && $check['user_group']==2){
      ...
      if(empty($userCmp) || ($userCmp['left_date']=="" || $userCmp['left_date']<time())){
         ...
         if(!empty($newQr)){
           ...
           $cmp = $this->app->getbyMid(end($string));
           $loginid = $cmp['id'];
         ...
         if($cmp_group==1){
           ...
           if(count($uCheck)==0){
              ...
              if($req){
                 ...
                 if($res){
                  $sendRes = array('msg'=>'Company Added','status'=>'1');
                  echo $response= json_encode(array('checkon'=>$sendRes));
                 }
              }
           }else{
     ...
     if(!empty($check['id']) && $check['user_group']==1 && $check['prime_att']==1){
       ...
       if(!empty($users_data)){
         ...
         foreach($users_data as $user){
             if(($user->doj!="" || $start_time>=$user->doj) && ($user->left_date=="" || $start_time<$user->left_date) && $user->hostel==$hostel){
                 $user_at = $this->app->getUserAttendanceByDate($start_time,$end_time,$user->user_id,$check['id'],1,$hostel);
...
function getCompanyUsersByStatus(){
 ...
 function updateBusinessGroup(){
   ...
   if(key($data)=="checkon"){
     $check=$this->app->checkMobile($data->checkon->mobile);
     if(!empty($check['id']) && $check['user_group']==1){
       $group = $this->app->updateBusinessGroup($data->checkon->id,$data->checkon->group_name,$data->checkon->start_time,$data->checkon->end_time,$data->checkon->weekly_off);
       if($group){
 ...
 function updateEmpProfile(){
   ...
   if(key($data)=="checkon"){
     $check=$this->app->checkMobile($data->checkon->mobile);
     if(!empty($check['id']) && $check['user_group']==1){
       $doj = $this->app->updateDoj($data->checkon->uid,$check['id'],strtotime($data->checkon->doj));
     ...
     if(!empty($check['id']) && $check['user_group']==1){
       ...
       if(!empty($newQr)){
         ...
         if($check['id']==$userCmp['business_id']){
           ...
           $res_mode = $this->app->getUserAttendanceByDate($start_time,$end_time,$loginid,$check['id'],1,0);
     ...
     if(!empty($check['id']) && $check['user_group']==2 && $check['manager']==1){
       ...
       if(!empty($newQr)){
         ...
         if($check['company']==$userCmp['business_id']){
           ...
           $start_time = strtotime(date("d-m-Y 00:00:00"));
           $end_time = strtotime(date("d-m-Y 23:59:59"));
           $res_mode = $this->app->getUserAttendanceByDate($start_time,$end_time,$loginid,$userCmp['business_id'],1,0);
      ...
      if(!empty($check['id']) && $check['user_group']==2){
        $userCmp = $this->app->getUserCompany($check['id']);
          ...
          if($user_status['user_status']=="1"){
            ...
            $res = $this->app->updateEmpLeave($data->checkon->id,$from_date,$to_date,$data->checkon->reason);
            if($res == 1){
              $sendRes = array('msg'=>'Leave requested Successfully','status'=>'1');
            }else{
              $sendRes = array('msg'=>'Failed request Leave','status'=>'0');
...
public function get_assigned_class_by_teacher() {
    ...
    $assigned_classes = $this->app->getAllAssignedClassesByTeacher($teacher_id);
...
public function students_daily_report_by_teacher()
{
	...
	$cmpName = $this->app->getBusinessById($loginId);
	

	// Fetch timetable entries for this teacher on the specific day
	$day_number = date('w', strtotime($selected_date));
	$periods = $this->app->getperiodTimeByteacher($period, $day_number, $teacher_id);



	$period_name = $periods->start_time . ' - ' . $periods->end_time;
	$section_id = (int)$periods->section;
	$dept_id = (int)$periods->dept;
	$semester_id = (int)$periods->semester_id;
 	$section_name=$this->app->getsectionById($section_id);
 	$dep_name=$this->app->getBusinessDepById($dept_id);
 	$semester_name=$this->app->getSemesterById($semester_id);
	$subject_id=$this->app->getsubjectnamebyid($periods->subject);
	$class_id=$this->app->getclassById($periods->class_room);
	...
	$students = $this->app->getSchoolStudentListbysection_new_api($section_id,$dept_id,$semester_id);
	...
	//    foreach ($periods as $period) {
			...
			$holiday = $this->app->getHolidayByBusinessId_new($loginId, strtotime($selected_date));
			...
			if ($holiday) {
				...
				$attendance = $this->app->getStudentAttendanceReportByDate($start_time, $end_time, $student->id, $loginId);
		...
		$response_data[] = [
			...
			'student_enroll_id' => $student->enroll_id,
	...
	echo json_encode([
		...
		'selected_date' => $selected_date,
         'section_id'	=>$section_name['name'],
          'dept_id'	=>	$dep_name['name'],
           'session_id'	=>	$semester_name['semestar_name'],
           'subject'	=>$subject_id['name'],
		'class_room'	=>$class_id['name'],
...
public function get_all_periods_api() {
    ...
    try {
        $periods = $this->app->getAllPeriods($bid);
        if (!is_array($periods)) {
  	...
  	function userCmpAttDetail(){
  $data=json_decode(file_get_contents("php://input"));
  if(key($data)=="checkon"){
    $check=$this->app->checkMobile($data->checkon->mobile);
    if(!empty($check['id']) && $check['user_group']==2){
	$userCmp = $this->app->getUserCompany($check['id']);
	$res = $this->app->userCmpDetail($check['id'],$userCmp['business_id']);
      ...
      if(!empty($res)){
       ...
       // foreach($res as $res){
          ...
          $data = array(
            ...
            'Qr_option' => $res['qr'] ,
            'Gps_Option'  => $res['gps'],
            'Selfie_Option'  => $res['face'],
             'field_duty_Option'  => $res['field_duty'],
              'selfie_with_gps_option'  => $res['selfie_with_gps'],
              'selfie_with_field_duty_option'  => $res['selfie_with_field_duty'],
               'team_att_Option'  => $res['colleague']
          );
       // }
        echo $response= json_encode(array('checkon'=>$data));
      }else{
 ...
 function getAttendancenew(){
  ...
  public function access_report(){
		if(!empty($this->session->userdata('id'))){
				...
				$config['total_rows'] = $this->web->countVisitorLogs($start_time, $end_time, $bio, $event_name, $search);
				$config['per_page'] = 50;
