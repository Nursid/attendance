...
class Api_new extends CI_Controller {
	function __construct(){
        parent::__construct();
		$this->load->database();
		$this->load->model('Api_Model_new','app');
	...
	function getAttendance(){
		...
		if(key($data)=="checkon"){
			...
			if(!empty($check['id']) && $check['user_group']==2){
				if(!empty($check['company'])){
					$company = $this->app->getbussnames($check['company']);
					$user_status = $this->app->userCmpStatus($check['id'],$check['company']);
					
          $startMonth = $this->app->attStartMonth($check['id']);

          $holidays = $this->app->getHoliday($check['company']);
	...
	function addUserAttendance(){
		...
		if(key($data)=="checkon"){
			...
			if(!empty($check['id']) && $check['user_group']==2){
				...
				$user_status = $this->app->userCmpStatus($check['id'],$cmp['id']);
				if( !empty($check['company']) && $check['company']==$cmp['id']){
					if($user_status['user_status']=="1"){
						$data = array(
							'bussiness_id'=>$cmp['id'],
							'user_id'=>$check['id'],
			...
			if(!empty($check['id']) && $check['user_group']==2){
				if(empty($check['company'])){
					...
					if($cmp['user_group']==1){
						...
						if($req){
							$res = $this->app->updateUserCompany($check['id'],$cmp['id'],strtotime(date('d-m-Y')));
							if($res){
    ...
    $data=json_decode(file_get_contents("php://input"));
        ...
        if(!empty($users_data)){
          ...
          foreach($users_data as $user){
              if($user->doj!="" && $start_time>=$user->doj){
                  $user_at = $this->app->getUserAttendanceByDate($start_time,$end_time,$user->user_id,$check['id']);
                $data = array();
  ...
  function updateEmpProfile(){
    $data=json_decode(file_get_contents("php://input"));
		if(key($data)=="checkon"){
			$check=$this->app->checkMobile($data->checkon->mobile);
			if(!empty($check['id']) && $check['user_group']==1){
        $emp = $this->app->updateEmpProfile($data->checkon->uid,$data->checkon->name,$data->checkon->address,$data->checkon->email,$data->checkon->group,$data->checkon->designation,$data->checkon->dob,$data->checkon->gender,strtotime($data->checkon->doj),$data->checkon->education);
        if($emp){
          ...
          foreach($users_data as $user){
            ...
            for($d=0; $d<$num_month;$d++){
              ...
              if($user->doj!="" && $new_start_time>=$user->doj){
                  $user_at = $this->app->getUserAttendanceByDate($new_start_time,$new_end_time,$user->user_id,$check['id']);
                  $data = array();
