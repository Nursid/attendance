...
class Api_v11 extends CI_Controller {
    ...
    function checkUserRoleStatus(){
     ...
     function addGpsTracking(){
      ...
      if(key($data)=="checkon"){
        ...
        $sendRes = array('msg'=>$msg,'status'=>$status,'offline'=>$newOffline);
        $response= json_encode(array('checkon'=>$sendRes));
        echo $response;
      }else{
          $msg = 'Please Add Company';
...
function getCompanyUsersAtt(){
  // $data=json_decode(file_get_contents("php://input"));
   //if(key($data)=="checkon"){
    $postdata=$this->input->post();
	$start_date = $postdata['start_date'];
	$end_date = $postdata['end_date'];
	$mobile = $postdata['midapp_id'];
	$check=$this->app->checkMobile($mobile);
       ...
       if(!empty($users_data)){
         ...
         foreach($users_data as $user){
             if(($user->doj!="" || $start_time>=$user->doj) && ($user->left_date=="" || $start_time<$user->left_date)){
               ...
               if(!empty($user_at)){
                 foreach($user_at as $at){
                     $devicesn=$this->app->getdevicebyid($at->device);
                   $data[] = array(
                    'id'=>$at->id,
                     'mode'=>$at->mode,
                     'date'=>date('Y-m-d', $at->io_time),
                     'time'=>date('h:i A', $at->io_time),
                     'latitude'=>$at->latitude,
                     'longitude'=>$at->longitude,
					 'location'=>$at->location,
         ...
         echo $response= json_encode($new_array);
       }else{
         $res=array('msg'=>'No Data Foud');
         echo $response= json_encode(array('checkon'=>$res));
       }
  ...
  // $data=json_decode(file_get_contents("php://input"));
   ...
   $getdata=$this->input->get();
    ...
    // if(!empty($check['id']) && $check['user_group']==1){
       $users_data = $this->app->getCompanyUsers($check['id']);
       $start_time = strtotime(date("Y-m-d h:i:00",strtotime($start_date)));
       $end_time = strtotime(date("Y-m-d h:i:59",strtotime($end_date)));
       ...
       if(!empty($users_data)){
             ...
             if(($user->doj!="" || $start_time>=$user->doj) && ($user->left_date=="" || $start_time<$user->left_date)){
               ...
               if(!empty($user_at)){
                 foreach($user_at as $at){
                     $devicesn=$this->app->getdevicebyid($at->device);
                     $manual=$at->manual;
                     if($manual=4){$type="Bio"; }else{ $type="App";}
         ...
         $users_empcode = $this->app->getCompanyUsersbyempcode($Empcode,$check['id']);
        if($users_empcode!=0){
             echo $response=$users_empcode[0]->name;
             echo $response= json_encode(array('checkon'=>$res));
         } else{
         
         $res=array('msg'=>'Wrong Emp Code','prime'=>'0');
         echo $response= json_encode(array('checkon'=>$res));
         
         
     }
     $res=array('msg'=>'Select Emp Code','prime'=>'0');
         echo $response= json_encode(array('checkon'=>$res));
     }
 ...
 public function getStudentBioAtt (){
          ...
          $ch = curl_init();
           ...
           $postData = 'TXN_NAME=GetAttendanceLog&DATA={"DeviceSlNo":"'.$device.'","FromDate":"'.$start_date.'","ToDate":"'.$end_date.'"}';
              ...
              curl_setopt($ch, CURLOPT_HTTPHEADER, $headers);
            print  $result = json_decode(curl_exec($ch));
