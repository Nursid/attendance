...
class Api_v14 extends CI_Controller {
...
////arpit
 ...
 function addStudentAttendance(){
   ...
   function getGpsAllLocationByUserId(){
    $data=json_decode(file_get_contents("php://input"));
    if(key($data)=="checkon"){
      $check=$this->app->checkMobile($data->checkon->mobile);
       if($check['validity']!="" && $check['validity']>time()){
      ...
      if(!empty($check['id']) && $check['user_group']==1 && $check['prime_att']==1){
        ...
        if(!empty($users_data)){
          $offline = $this->app->getUserGpsLocationByDate($start_time,$end_time,$data->checkon->empId,$check['id']);
          $newOffline = array();
          foreach($offline as $at){
            $newOffline[]=array(
              "id"=>$at->id,
   ...
   public function userspunchlog(){
      //$data=$this->input->post();
      $postdata=$this->input->post();
      $start_date = $postdata['start_date'];
      $end_date = $postdata['end_date'];
 	$mobile = $postdata['device'];
      $device =$_POST['device'];
    ...
    $postData = 'TXN_NAME=GetAttendanceLog&DATA={"DeviceSlNo":"'.$device.'","FromDate":"'.$start_date.'","ToDate": "'.$end_date.'"}';
