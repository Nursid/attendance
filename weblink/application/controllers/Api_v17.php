...
class Api_v17 extends CI_Controller {
    ...
    if(key($data)=="checkon"){
      ...
      if(!empty($check['id']) && $check['user_group']==1 && $check['prime_att']==1){
        ...
        if(!empty($users_data)){
          ...
          echo $response= json_encode(array('checkon'=>$newOffline));
        }else{
          $res=array('msg'=>'No Data Foud');
          echo $response= json_encode(array('checkon'=>$res));
        }
    ...
    if(key($data)=="checkon"){
      ...
      if(!empty($check['id']) && $check['user_group']==1 && $check['prime_att']==1){
        ...
        if(!empty($users_data)){
          ...
          echo $response= json_encode(array('checkon'=>$new_array,'groups'=>$groups,'holidays'=>$holidays,'user_request'=>$users_data,'prime'=>'1','sections'=>$sections,'tasks'=>$tasks,'rules'=>$rules,'cmpOptions'=>$getCmpOptions));
        }else{
          $res=array('msg'=>'No Data Foud');
          echo $response= json_encode(array('checkon'=>$res));
        }
    ...
    if(key($data)=="checkon"){
      if($data->checkon->key=="752d2c9ecfe079e5e5f3539f4d750e5c"){
       ...
       $gp = $this->web->getBusinessGroupByUserId($getUserByBioId['business_group']);
        ...
        if( !empty($userCmp['business_id']) && $userCmp['business_id']==$loginid){
         $checkOffline = $this->app->checkIoTime($uid,$loginid,$io_timenew, $io_timedubb);
...
curl_close($ch);
    ...
    curl_setopt_array($ch, [
        CURLOPT_URL => "https://attendance.aadigyan.in/API/Attendance.asmx/PostAttendance",
...
// vistor mode api
   ...
   function addBioAttendanceaug(){
    ...
    $url = 'https://app.aadigyan.in/api/attendance.asmx/PostAttendance';
    ...
    if(curl_errno($ch)) {
        ...
        //$sendRes = array('msg'=>'oter Company bioid','status'=>$result);
        // echo $response2= json_encode(array('checkon'=>$sendRes));
 ...
 public function addbiometricusername(){
   ...
   {  
     ...
     $uname = $this->web->getNameByUserId($stids);
       $uid=$uname[0]->bio_id;
       $name = $uname[0]->name;
    $ch = curl_init();
        $headers  = ['Content-Type: application/x-www-form-urlencoded'];
  		
   $postData = 'TXN_NAME=AddUser&DATA={"DeviceSlNo":"'.$device.'","UserID":"'.$uid.'","Name":"'.$name.'"}';
 ...
 public function deletebiouser(){
   ...
   $postData = 'TXN_NAME=DeleteUser&DATA={"DeviceSlNo":"'.$device.'","UserId":"'.$uid.'"}';
        curl_setopt($ch, CURLOPT_URL,"http://103.30.72.34:7788/WebService.asmx/liveEmsTransaction");
        curl_setopt($ch, CURLOPT_POST, 1);
        curl_setopt($ch, CURLOPT_RETURNTRANSFER, 1);
        curl_setopt($ch, CURLOPT_POSTFIELDS, $postData);         
   ...
   public function addbiometricusercard(){
      $data=$this->input->post();
      $stid= $_POST['stid'];
...
$postData = 'TXN_NAME=SetCardData&DATA={"DeviceSlNo":"'.$device.'","UserID":"'.$uid.'","Name":"'.$name.'","Admin":"0","Card":"'.$card.'
","PWD":""}';
          curl_setopt($ch, CURLOPT_URL,"http://103.30.72.34:7788/WebService.asmx/liveEmsTransaction");
          curl_setopt($ch, CURLOPT_POST, 1);
          curl_setopt($ch, CURLOPT_RETURNTRANSFER, 1);
      ...
      public function disablebiometricuser(){
        $data=$this->input->post();
       $stid= $_POST['stid'];
  ...
  $postData = 'TXN_NAME=EnableUser&DATA={"DeviceSlNo":"'.$device.'","UserId":"'.$uid.'","Status":"Disable"}';
            curl_setopt($ch, CURLOPT_URL,"http://103.30.72.34:7788/WebService.asmx/liveEmsTransaction");
            curl_setopt($ch, CURLOPT_POST, 1);
            curl_setopt($ch, CURLOPT_RETURNTRANSFER, 1);
            curl_setopt($ch, CURLOPT_POSTFIELDS, $postData);         
  ...
  public function enablebiometricuser(){
        $data=$this->input->post();
       $stid= $_POST['stid'];
...
$postData = 'TXN_NAME=GetCardData&DATA={"DeviceSlNo":"'.$device.'"}';
          curl_setopt($ch, CURLOPT_URL,"http://103.30.72.34:7788/WebService.asmx/liveEmsTransaction");
          curl_setopt($ch, CURLOPT_POST, 1);
          curl_setopt($ch, CURLOPT_RETURNTRANSFER, 1);
          curl_setopt($ch, CURLOPT_POSTFIELDS, $postData);         
   ...
   public function getuserpunchdetail(){
      $data=$this->input->post();
      $device =$_POST['device'];
...
$postData = 'TXN_NAME=GetAttendanceLog&DATA={"DeviceSlNo":"'.$device.'","FromDate":"'.$FromDate.'","ToDate": "'.$ToDate.'"}';
       ...
       if($result->Status=="Success" && !empty($result->data)){
            foreach($result->data as $row){
             // $getUserByBioId = $this->app->getUserByBioId($row->EnrollmentNo,$buid);
             // if(isset($getUserByBioId)){
                //$userCmp = $this->app->getUserCompany($getUserByBioId['id']);
       
            //  $user_id=$getUserByBioId['name'];
             echo "ID:-". $user_id=$row->EnrollmentNo."  ,  ";
       
            echo "Time".$io_time=$row->PunchDateTime." <br> ";
            // print_r($user_id);
             // print_r($io_time);
             // }
            //    
  ...
  function transferBioAttendance(){
    $data=$this->input->post();
    //$device =$_POST['device'];
    $dev_id= $_POST['device'];
     $buid =$_POST['buid'];
             ...
             $punchtime=$row->PunchDateTime;
              ...
              if(isset($getUserByBioId)){
                ...
                if( !empty($userCmp['business_id']) && $userCmp['business_id']==$buid){
                  $checkOffline = $this->app->checkIoTime_old($getUserByBioId['id'],$buid,strtotime($row->PunchDateTime));
...
$postData = 'TXN_NAME=SetDeviceTime&DATA={"DeviceSlNo":"'.$device.'"}';
          curl_setopt($ch, CURLOPT_URL,"http://103.30.72.34:7788/WebService.asmx/liveEmsTransaction");
          curl_setopt($ch, CURLOPT_POST, 1);
          curl_setopt($ch, CURLOPT_RETURNTRANSFER, 1);
          curl_setopt($ch, CURLOPT_POSTFIELDS, $postData);         
   ...
   public function clearalluser(){
      $data=$this->input->post();
      $device =$_POST['device'];
      $ch = curl_init();
          $headers  = ['Content-Type: application/x-www-form-urlencoded'];
    	
$postData = 'TXN_NAME=ClearAllUser&DATA={"DeviceSlNo":"'.$device.'"}';
          curl_setopt($ch, CURLOPT_URL,"http://103.30.72.34:7788/WebService.asmx/liveEmsTransaction");
          curl_setopt($ch, CURLOPT_POST, 1);
          curl_setopt($ch, CURLOPT_RETURNTRANSFER, 1);
          curl_setopt($ch, CURLOPT_POSTFIELDS, $postData);         
      ...
      public function clearBioAdmin(){
      $data=$this->input->post();
      $device =$_POST['device'];
      $ch = curl_init();
          $headers  = ['Content-Type: application/x-www-form-urlencoded'];
    	
$postData = 'TXN_NAME=ClearAdmin&DATA={"DeviceSlNo":"'.$device.'"}';
          curl_setopt($ch, CURLOPT_URL,"http://103.30.72.34:7788/WebService.asmx/liveEmsTransaction");
          curl_setopt($ch, CURLOPT_POST, 1);
          curl_setopt($ch, CURLOPT_RETURNTRANSFER, 1);
          curl_setopt($ch, CURLOPT_POSTFIELDS, $postData);         
...
$postData = 'TXN_NAME=ClearAllAttendanceLog&DATA={"DeviceSlNo":"'.$device.'"}';
          curl_setopt($ch, CURLOPT_URL,"http://103.30.72.34:7788/WebService.asmx/liveEmsTransaction");
          curl_setopt($ch, CURLOPT_POST, 1);
          curl_setopt($ch, CURLOPT_RETURNTRANSFER, 1);
          curl_setopt($ch, CURLOPT_POSTFIELDS, $postData);         
    ...
    function addBiometricAttendance(){
    $data=json_decode(file_get_contents("php://input"));
    if($data->checkon->key=="752d2c9ecfe079e5e5f3539f4d750e5c" || true){
      $sn = $data->sn;
      foreach($data->record as $row){
        ...
        if(isset($row->note->location)){
            $location = $row->note->location;    
        }
        if(isset($row->note->satellites)){
            $satellites = $row->note->satellites;
        }
        if(isset($row->note->msg)){
            $msg = $row->note->msg;
        }
        if(!empty($location)){
          ...
          $lat = $location[0];
          $lon = $location[1];
          $address = $this->latLongToAddress($lat,$lon);
        }
        $image = "";
        if(isset($row->image)){
         $image = $row->image;   
        ...
        if(!empty($image)){
          ...
          $imgs = imagecreatefromstring($img);
          if($imgs != false){
            $date = strtotime(date('Y-m-d h:i:s')).$row->enrollid;
            $image="upload/visitor/$date.jpg";
            imagejpeg($imgs, "$image");
          }
        }
        
        $getBusinessByDeviceId = $this->app->getBusinessByDeviceId($sn);
        if(isset($getBusinessByDeviceId)){
          $loginids = $getBusinessByDeviceId['bid'];
          $deviceMode = $getBusinessByDeviceId['mode'];
          $deviceid = $getBusinessByDeviceId['id'];
        }
        $io_time=strtotime($row->time);
         $io_timedubb=$io_time-180;
        if($deviceMode==12){
        $uid=$row->enrollid;
        $getUserByBioId = $this->app->getUserByBioId($uid,$loginids);
        if(isset($getUserByBioId)){
      ...
      $userCmp = $this->app->getUserCompany($usid);
       //if( !empty($userCmp['business_id']) && $userCmp['business_id']==$loginids){ 
           ...
           // $checkOffline = $this->app->checkIoTime($usid,$loginids,$io_time, $io_timedubb);
            
          // if(empty($checkOffline)){
           // $start_time = strtotime(date("d-m-Y 00:00:00",$io_time));
           // $end_time = strtotime(date("d-m-Y 23:59:59",$io_time));

           $mode = "out";  
             $eventid=$row->event;
            $eventmode = $this->web->getEventById($eventid);
            if(!empty($eventmode)){
                $eventname=$eventmode[0]->name;
       ...
       $data = array(
            ...
            $ress = $this->app->insertAttendance($data);
         //   if($res == 1){
        // $sendRes = array('msg'=>'new log added Successfully','status'=>'1');
          //  }else{
            //  $sendRes = array('msg'=>'Failed to Add log','status'=>'0');
           // }
    //   } offline att
      //  } userid
        }elseif($deviceMode==10){
        ...
        $ress = $this->web->insertvisitorlog($visitor);
  ...
  private function latLongToAddress($latitude,$longitude){
    $curl = curl_init();
    curl_setopt_array($curl, array(
    CURLOPT_URL => 'https://maps.googleapis.com/maps/api/geocode/json?latlng='.$latitude.','.$longitude.'&key=AIzaSyDo0-Z1g1hqLImEPIjmXxtQfpntWfufSfA',
    CURLOPT_RETURNTRANSFER => true,
    CURLOPT_ENCODING => '',
    ...
    try {
        if(!empty($response)){
            ...
            $address = $resultJson->results[0]->formatted_address;
        }
    } catch (Exception $err) {
        $address="";
    }
  ...
  function transferStuBioAttendance(){
    $data=$this->input->post();
    //$device =$_POST['device'];
    $dev_id= $_POST['device'];
     $buid =$_POST['buid'];
  ...
  //  $model =$_POST['model'];
      ...
      //$buid = $this->session->userdata('login_id');
       ...
       $postData = 'TXN_NAME=GetAttendanceLog&DATA={"DeviceSlNo":"'.$device.'","FromDate":"'.$FromDate.'","ToDate":"'.$ToDate.'"}';
          ...
          if($result->Status=="Success" && !empty($result->data)){
            ...
            foreach($result->data as $row){
           // $enroll_id=(int)$row->EnrollmentNo;
            $enroll_id=$row->EnrollmentNo;
             $punchtime=$row->PunchDateTime;
          
          
           $getStudentByBioId = $this->web->getStudentByBioId($enroll_id,$buid);
      if(isset($getStudentByBioId)){
        $stuid = $getStudentByBioId['id'];
         $name= $getStudentByBioId['name'];
         
         $checkOffline = $this->app->checkIoTime_Stu($stuid,$buid,strtotime($row->PunchDateTime));
                  echo " Found Data: ".$count2." Enroll Id: ".$enroll_id." Time: ". $punchtime ;
                  if(empty($checkOffline)){
                        echo "(New Data Found) <br>" ; 
     ...
     $datas = array(
        ...
        'student_status'=> "1",
        'device'=> $dev_id,
       'time'=>strtotime($row->PunchDateTime),
        'update_date'=>time(),
        'date_time'=>time()
      );
      $res = $this->web->insertstudentbioAttendance($datas);  
  ...
  public function insert_bio_log()
{
    ...
    foreach($result['record'] as $row)
    {
        ...
        if(isset($row['note']['location']) && !empty($row['note']['location']))
        {
            $location = $row['note']['location'];

            $gps = explode(',', $location);

            $latitude  = isset($gps[0]) ? trim($gps[0]) : '';
            $longitude = isset($gps[1]) ? trim($gps[1]) : '';
        }
        ...
        // User Details (for hostel/night etc.)
      //  $userCmp = $this->app->getUserById($usid);
      $userCmp = $this->app->getUserCompany($usid);
        $night = 0;
