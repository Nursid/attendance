...
class Api_v18 extends CI_Controller {
...
function getCompanyUsersByStatus(){
 ...
 function addEmpLeave(){
  ...
  function addOfflineAttendance(){
    ...
    //   }
      ...
      $sendRes = array('msg'=>$msg,'status'=>$status,'offline'=>$newOffline,'company_data'=>$cmp_array,'holiday'=>$holidays,'group'=>$groups,'leaves'=>$leaves,'logout'=>'0','sections'=>$sections,'user_section'=>$check['section'],'options'=>$options,'user_images'=>$users_images,'task'=>$tasks,'rules'=>$rules);
    ...
    if(key($data)=="checkon"){
      ...
      if(!empty($check['id']) && $check['user_group']==1 && $check['prime_att']==1){
        ...
        if(true){
          ...
          echo $response= json_encode(array('checkon'=>$new_array,'groups'=>$groups,'holidays'=>$holidays,'user_request'=>$users_data,'prime'=>'1','sections'=>$sections,'tasks'=>$tasks,'rules'=>$rules,'cmpOptions'=>$getCmpOptions,'departmentSection'=>$getDepartmentSections));
        }else{
          $res=array('msg'=>'No Data Foud');
          echo $response= json_encode(array('checkon'=>$res));
        }
 ...
 function getHostelDetails(){
  ...
  public function enablebiometricalluser(){
       ...
       $bid=$uname[0]->bid;
    //  $bid="5";
    echo  $device=$uname[0]->deviceid;
       $users = $this->web->getBioIdbycompany($bid);
  ...
  $postData = 'TXN_NAME=EnableUser&DATA={"DeviceSlNo":"'.$device.'","UserId":"'.$uid.'","Status":"Enable"}';
            curl_setopt($ch, CURLOPT_URL,"http://103.30.72.34:7788/WebService.asmx/liveEmsTransaction");
            curl_setopt($ch, CURLOPT_POST, 1);
            curl_setopt($ch, CURLOPT_RETURNTRANSFER, 1);
            curl_setopt($ch, CURLOPT_POSTFIELDS, $postData);         
  ...
  function transferBioAttendanceadmin(){
    $data=$this->input->post();
    $dev_id= $_POST['device'];
    $uname = $this->web->getbidbydeviceid($dev_id);
       $buid=$uname[0]->bid;
    ...
    $model=$uname[0]->model;
     ...
     $ToDate =date("Y-m-d",strtotime($_POST['end_date']));
      //$buid = $this->session->userdata('login_id');
     ...
     $postData = 'TXN_NAME=GetAttendanceLogOfflineDevice&DATA={"DeviceSlNo":"'.$device.'","FromDate":"'.$FromDate.'","ToDate":"'.$ToDate.'"}';
      ...
      $postData = 'TXN_NAME=GetAttendanceLog&DATA={"DeviceSlNo":"'.$device.'","FromDate":"'.$FromDate.'","ToDate":"'.$ToDate.'"}';
          }
          curl_setopt($ch, CURLOPT_URL,"http://103.30.72.34:7788/WebService.asmx/liveEmsTransaction");
          curl_setopt($ch, CURLOPT_POST, 1);
          curl_setopt($ch, CURLOPT_RETURNTRANSFER, 1);
          ...
          if($result->Status=="Success" && !empty($result->data)){
            ...
            foreach($result->data as $row){
             ...
             $punchtime=$row->PunchDateTime;
              $getUserByBioId = $this->app->getUserByBioId($enroll_id,$buid);
              ...
              if(isset($getUserByBioId)){
                ...
                if( !empty($userCmp['business_id']) && $userCmp['business_id']==$buid){
                  $checkOffline = $this->app->checkIoTime($getUserByBioId['id'],$buid,strtotime($row->PunchDateTime));
                  echo " Found Data: ".$count2." Enroll Id: ".$enroll_id." Time: ". $punchtime ;
                  ...
                  $count++;
                  echo " New Data: ".$count." User Name: ".$name." Time: ".$punchtime."(Updated) <br>"; 
                  } else{
            ...
            echo $count2-1 ." Data Found <br> ";
        echo $count-1 ." New data Added ";
          } 
      
       
       // $res=array('msg'=>$Msg,'status'=>'1','data'=>$result->data);
        print_r($Msg);
        echo '<br/>';
     ...
     $ToDate =date("Y-m-d",strtotime($_POST['end_date']));
      //$buid = $this->session->userdata('login_id');
        ...
        $ch = curl_init();
          $headers  = ['Content-Type: application/x-www-form-urlencoded'];
          if($model=1){
     $postData = 'TXN_NAME=GetAttendanceLogOfflineDevice&DATA={"DeviceSlNo":"'. $dev_id.'","FromDate":"'.$FromDate.'","ToDate":"'.$ToDate.'"}';
          }else {
    ...
    if(key($data)=="checkon"){
      ...
      if($deviceMode==6){
        ...
        'TakeAttendance' => array(
            'enrollment_id' => "13",
            'device_token' => "AYTG23079725",
            'attendance_datetime' => "75765688888"
        )
    );
    
    $jsonData = json_encode($data);
    $url = 'https://attendance.aadigyan.in/API/Attendance.asmx/PostAttendance';
    $ch = curl_init();
    curl_setopt($ch, CURLOPT_URL, $url);
    curl_setopt($ch, CURLOPT_POST, 1);
...
curl_close($ch);
   ...
   // return array('success' => true, 'http_code' => $httpCode, 'response' => $response);
...
if(key($data)=="checkon"){
   ...
   //   $data_loc=explode(",",$location);
    ...
    //     !isset($input['device_token']) ||
    //     !isset($input['attendance_datetime'])) {
    ...
    $postData = [
        "enrollment_id" => "16",
        "device_token" => "AYTG23079725",
        "attendance_datetime" => "12/02/2026 11:32:58"
    ];
