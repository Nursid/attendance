      ...
      // $otp=rand(1000,9999);
       ...
       if(isset($data->checkon->linked)){
        ...
        $msg="Dear User, Your mobile verification code for login in MID App is $otp. Please Use this code to verify your mobile No.";
         ...
         if($mobile=="1234567890" || $mobile=="1234567888" || $mobile=="0888088808" || $mobile=="8188801333"){
             $otp="8888";
         }
         $data=array(
           'otp'=>$otp,
...
$image=$data->checkon->offerimage;
 ...
 // $url="http://151.106.8.79/sendsms/bulksms.php?username=checkon&password=checkon&type=TEXT&sender=MIDAPP&mobile=$mobile&entityId=1301160517752005345&templateId=1307161872754873036&message=".urlencode($msg);
 //$url="http://5.189.187.82/sendsms/bulk.php?username=sigmait&password=hCmZWAVB&type=TEXT&sender=MIDAPP&mobile=$mobile&message=urlencode($msg)&entityId=1301160517752005345&templateId=1307161872754873036";
 $ch = curl_init($url);
    ...
    function checkUserRoleStatus(){
     ...
     function addGpsTracking(){
      ...
      if(key($data)=="checkon_stop"){
        ...
        $offline = $this->app->getUserGpsTrackingAll($check['id']);
        $newOffline = array();
        foreach($offline as $at){
            $newOffline[]=array(
                  "id"=>$at->id,
      ...
      //  $gpsOffline = $this->app->getUserGpsLocationAll($check['id']);
        $newGpsOffline = array();
        foreach($gpsOffline as $at){
            $newGpsOffline[]=array(
              "id"=>$at->id,
              "bid"=>$at->bid,
              "uid"=>$at->uid,
    ...
    if(key($data)=="checkon"){
      ...
      if(!empty($check['id'])){
          ...
          if($data->checkon->imageChanged=="1"){
           ...
           $res=$this->db->update('student',$studentData);
          if($res){
    ...
    if(key($data)=="checkon"){
      ...
      if(!empty($check['id']) && $check['user_group']==2){
        ...
        if(!empty($getTeacherById['id'])){
          ...
          $getCompanyStudents = $this->app->getParentStudents($check['mobile']);
          foreach($getCompanyStudents as $st){
        ...
        echo $response= json_encode(array('classes'=>$getCompanyClasses,'classTeacher'=>$getCompanyTeachers,'students'=>$getCompanyStudents,'studentAttendance'=>$getCompanyStudentAttendance));
      ...
      if(!empty($check['id']) && $check['user_group']==1){
       ...
       // foreach($getBioDevice as $device){
         ...
         $fromDate ="2024-07-15";
          ...
          $toDate = "2024-07-15";
          $device="2203110060";
          $postData = 'TXN_NAME=GetAttendanceLogOfflineDevice&DATA={"DeviceSlNo":"'.$device.'","FromDate":"'.$fromDate.'","ToDate":"'.$toDate.'"}';
          ...
          if($result->Status=="Success" && !empty($result->data)){
            foreach($result->data as $row){
              ...
              $getUserByBioId = $this->app->getUserByBioId($enroll_id,$check['id']);
