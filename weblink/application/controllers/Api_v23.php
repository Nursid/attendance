...
//use Google\Client;
class Api_v23 extends CI_Controller {
...
public function AddAppintment(){
   ...
   $bookingdate=date("D M d Y",strtotime($data->checkon->bookingdate));
   $bookingtime=$data->checkon->bookingtime;
   $loginid=$data->checkon->loginid;
   $bussinessid=$data->checkon->bussinessid;
   $departmentid=$data->checkon->departmentid;
   ...
   if(key($data)=="checkon"){
    if($bookingtime!=""){
        ...
        if($code!=""){
          $orderId = substr($code,'4')+1;
        }
        $stud_code = $tag.$orderId;
        $data=array(
         'bussiness_id'=>$bussinessid,
         'bookingdate'=>$bookingdate,
         'booking_time'=>$bookingtime,
         ...
         'name'=>$data->checkon->name,
         'mobile'=>$data->checkon->mobile,
         'relation'=>$data->checkon->relation,
         'address'=>$data->checkon->address,
         'slot'=>$data->checkon->slot,
         'date'=>time()
        );
        $checkBookingTimeSlot = $this->app->checkBookingTimeSlot($bussinessid,$bookingdate,$bookingtime);
        if(empty($checkBookingTimeSlot)){
            $update=$this->db->insert('book_appointment',$data);
            if($update > 0){
              $response=array('msg'=>'Booking Done !');
              echo $response= json_encode(array('checkon'=>$response));
            ...
            $response=array('msg'=>'Time Slot not Available !');
            echo $response= json_encode(array('checkon'=>$response));
        }
    }else{
        $response=array('msg'=>'Please Select Time Slot');
        echo $response= json_encode(array('checkon'=>$response));
    }
   }
 }
...
public function getappointment(){
    ...
    $departmentid=$data->checkon->departmentid;
    $subdepartmentid=$data->checkon->subdepartmentid;
    $day=date("D M d Y",strtotime($data->checkon->day));
    if(key($data)=="checkon"){
      $getdepart=$this->app->getappointmenttime($loginid,$departmentid,$subdepartmentid);
      $userdetails=$this->app->userdetails($loginid);

      $open_time = date('g:i A');
      $slotdifference=$getdepart['slot_diff'];
      $currenttime=date('g:i A');
      $currentdated=date('D M d Y');
      $dayofweek = date('N', strtotime($day));
      $bookedSlots = $this->app->getbookedtime($loginid,$departmentid,$subdepartmentid,$day);
      $time_slots = array();
      if(!empty($getdepart['shiftList'])){
        $shiftList = json_decode($getdepart['shiftList']);
        if(!empty($shiftList) && !empty($shiftList[$dayofweek-1]->shift) && $shiftList[$dayofweek-1]->isOpen){
          $open_time = date('g:i A',  $shiftList[$dayofweek-1]->shift[0]->openTime);
          foreach($shiftList[$dayofweek-1]->shift as $shift){
            if($shift->openTime!=$shift->closeTime){
              $slots = $this->prepare(date('g:i A',$shift->openTime),date('g:i A',$shift->closeTime), $slotdifference);
              foreach($slots as $sl){
                $booked = array_search($sl,array_column($bookedSlots,'booking_time'));
                $timeSt = array(
                  "time"=>$sl,
                  "status"=>1,
                  "slot"=>count($time_slots)+1
                );
                if(!is_bool($booked)){
                  $timeSt = array(
                    "time"=>$sl,
      ...
      if($day==$currentdated){
        //$time_slots = array_diff($time_slots,$curr);
      }
      $qw=array_values($time_slots);
      $ss=count($qw);
      $morning=array();
      $afternoon=array();
      $evening=array();
      for($i=0; $i<$ss;$i++){
        $prevContains = false;
        if($day==$currentdated){
            $prevSlot = array_search($qw[$i]['time'],$curr);
            if(!is_bool($prevSlot)){
              $prevContains = true;
    		}
        }
        if(!$prevContains){
            if($qw[$i]['time']<"12:00 PM"){
              $morning[]=$qw[$i];
            }elseif($qw[$i]['time']>="12:00 PM" && $qw[$i]['time']<"16:00 PM"){
              $afternoon[]=$qw[$i];
            }else{
              $evening[]=$qw[$i];
            }
   ...
   if(key($data)=="checkon"){
     ...
     foreach($dep as $data)
     {
       ...
       $subdepartment=$this->app->getsubdeptnew($data->subdepartment);
        ...
        $canCancel = 1;
        if(strtotime($data->bookingdate." ".substr($data->booking_time,0,5))<time()){
          $canCancel = 0;
        }
       $dataws[]=array(
...
///Appointment Booking History
 ...
 public function Appointhistorybussiness(){
   ...
   if(key($data)=="checkon"){
     $dd=$data->checkon->loginid;
     $date = date("D M d Y",strtotime($data->checkon->date));
     $dep=$this->app->getAppointmentbussinessByDate($dd,$date);
     
     foreach($dep as $data){
       $bussinessname=$this->app->getbussnames($dd);
   ...
   if(key($data)=="checkon"){
     ...
     if(!empty($check['id']) && $check['user_group']==2){
      ...
      if(empty($userCmp) || ($userCmp['left_date']=="" || $userCmp['left_date']<time())){
         ...
         if($cmp_group==1){
            $getCmpOptions = $this->app->getCmpOptions($loginid);
            $insertData = array(
             'business_id'=>$loginid,
             'user_id'=>$check['id'],
             'doj'=>strtotime(date('d-m-Y')),
             'hostel'=>$data->checkon->hostel,
             'date'=>time()
           );
            if(isset($getCmpOptions[0])){
                $insertData = array(
                 'business_id'=>$loginid,
                 'user_id'=>$check['id'],
                 'doj'=>strtotime(date('d-m-Y')),
                 'hostel'=>$data->checkon->hostel,
                 'qr'=>$getCmpOptions[0]->qr,
                 'gps'=>$getCmpOptions[0]->gps,
                 'face'=>$getCmpOptions[0]->face,
                 'colleague'=>$getCmpOptions[0]->colleague,
                 'auto_gps'=>$getCmpOptions[0]->auto_gps,
                 'gps_tracking'=>$getCmpOptions[0]->gps_tracking,
                 'field_duty'=>$getCmpOptions[0]->field_duty,
                 'four_layer_security'=>$getCmpOptions[0]->four_layer_security,
                 'selfie_with_gps'=>$getCmpOptions[0]->selfie_with_gps,
                 'selfie_with_field_duty'=>$getCmpOptions[0]->selfie_with_field_duty,
                 'qr_mode'=>$getCmpOptions[0]->qr_mode,
                 'date'=>time()
               );
            }
           ...
           if(count($uCheck)==0){
              $req = $this->app->addUserCmpStatus($insertData);
              if($req){
                 $res = $this->app->updateUserCompany($check['id'],$loginid,strtotime(date('d-m-Y')));
                 $res = $this->app->updateUserDojHostel($check['id'],$loginid,'',strtotime(date('d-m-Y')),$data->checkon->hostel);
                 if($res){
...
function getCompanyUsersByStatus(){
     ...
     if(!empty($check['id']) && $check['user_group']==1){
       ...
       if(!empty($emp)){
         $data = array(
           ...
           "rule_id"=>$emp->rule_id,
           "qr_mode"=>$emp->qr_mode,
           "month_weekly_off"=>$emp->month_weekly_off,
           "rules"=>$rules
         );
         echo $response= json_encode(array('checkon'=>$data));
       }else{
   ...
   if(key($data)=="checkon"){
     ...
     if(!empty($check['id']) && $check['user_group']==1){
       $data = array(
         ...
         'month_weekly_off'=>$data->checkon->month_weekly_off
       );
 ...
 function updateBusinessGroup(){
   ...
   if(key($data)=="checkon"){
     $check=$this->app->checkMobile($data->checkon->mobile);
     if(!empty($check['id']) && $check['user_group']==1){
       $group = $this->app->updateBusinessGroup($data->checkon->id,$data->checkon->group_name,$data->checkon->start_time,$data->checkon->end_time,$data->checkon->weekly_off,$data->checkon->day_start_time,$data->checkon->day_end_time,$data->checkon->month_weekly_off);
       if($group){
          ...
          foreach($users_data as $user){
            if($user->hostel=="0"){
             ...
             $leaves = $this->app->getEmpLeaves($user->user_id);
              ...
              $rules = $this->app->getRule($check['id'],$user->rule_id);
              $mispunch = "0";
              $ca_wo_lofi = "0";
              $mark_ab_week = "0";
              $ov_shift = "0";
              ...
              for($d=0; $d<$num_month;$d++){
                ...
                $day_sub_status="";
                if(($user->doj!="" || $start_time>=$user->doj) && ($user->left_date=="" || $start_time<$user->left_date)){
                    // $getDayReport = $this->app->getDayReport($check['id'],$user->user_id,$new_start_time,$new_end_time);
                    // if(!empty($getDayReport)){
                    //     $user_at = json_decode($getDayReport['data']);
                    //   }else{
                  ...
                  //     "day_start_time"=>$new_start_time,
                  //     "day_end_time"=>$new_end_time,
                  //     'date_time'=>time()
                  //   );
                  //   if(empty($getDayReport)){
                  //     $this->app->addDayReport($insertDayData);
                  //   }
                    $months_array[] = array(
 ...
 function addEmpLeave(){
  ...
  function activateLicence(){
    ...
    if(key($mid)=="checkon"){
      ...
      if(!empty($check['id'])){
        ...
        if($res){
          $validity;
          if($res["licence"]==1 || $res["licence"]==2 || $res["licence"]==3){
            if($check['validity']!="" && $check['validity']>time()){
            ...
            if($res["licence"]==2){
              $maxEmp=10;
            }else if($res["licence"]==3 || $res["licence"]==1){
              $maxEmp=25;
            }
            $assign = $this->app->updateMaxEmp($check['id'],$maxEmp);
            $assign = $this->app->updateValidity($check['id'],time(),$validity);
            $assign = $this->app->assignNewQR($check['id'],end($string));
            if($assign){
              $sendRes = array('msg'=>'Base Pack Activated Successfully','status'=>'1');
              echo $response= json_encode(array('checkon'=>$sendRes));
            }else{
              $sendRes = array('msg'=>'Failed to Activate Base Pack','status'=>'0');
              echo $response= json_encode(array('checkon'=>$sendRes));
            }
          }else{
            $sendRes = array('msg'=>'Base Pack not Found','status'=>'0');
            echo $response= json_encode(array('checkon'=>$sendRes));
          }
        }else{
          $sendRes = array('msg'=>'Base Pack not Found','status'=>'0');
  ...
  function boostLicence(){
    ...
    if(key($mid)=="checkon"){
      ...
      if(!empty($check['id'])){
        ...
        $res = $this->app->checkNewQR(end($string));
        if($res){
          if($res["licence"]==4 || $res["licence"]==5){
            $getCmpOptions = $this->app->getCmpOptions($check['id']);
            $maxEmp=0;
            if(isset($getCmpOptions[0])){
              $maxEmp=$getCmpOptions[0]->max_emp;
            }
            if($res["licence"]==4){
              $maxEmp+=10;
            ...
            $assign = $this->app->assignNewQR($check['id'],end($string));
            if($assign){
              $sendRes = array('msg'=>'Boost Pack Activated Successfully','status'=>'1');
              echo $response= json_encode(array('checkon'=>$sendRes));
            }else{
              $sendRes = array('msg'=>'Failed to Activate Boost Pack','status'=>'0');
              echo $response= json_encode(array('checkon'=>$sendRes));
            }
          }else{
            $sendRes = array('msg'=>'Boost Pack not Found','status'=>'0');
            echo $response= json_encode(array('checkon'=>$sendRes));
          }
        }else{
          $sendRes = array('msg'=>'Boost Pack not Found','status'=>'0');
  ...
  function addOfflineAttendance(){
    ...
    if(isset($data->checkon)){
      ...
      $offlineTime = $data->checkon->offlineTime;
      $check=$this->app->checkMobile($data->checkon->mobile);
      $groups = $this->app->getUserGroup($check['business_group']);
      ...
      $endTimestamp = strtotime(date("t-m-Y 12:59:59",$currentMonth));
      $userCmp = $this->app->getUserCompany($check['id']);
      //$check['device_id']==$data->checkon->device_id
      if(true){
        if(isset($userCmp) && ($userCmp['left_date']=="" || $userCmp['left_date']>time())){
        $userCmp = $this->app->getUserCompany($check['id']);
        $user_status = $this->app->userCmpStatus($check['id'],$userCmp['business_id']);
      ...
      foreach($list as $at){
        ...
        if(!empty($check['id']) && $check['user_group']==2){
          ...
          if( !empty($userCmp['business_id']) && $userCmp['business_id']==$at->business_id){
            if($user_status['user_status']=="1"){
              $checkOffline = $this->app->checkIoTime($check['id'],$at->business_id,$at->time);
              if(empty($checkOffline)){
                ...
                if(!empty($selfie)){
                    ...
                    $imgs = imagecreatefromstring($img);
                    if($imgs != false){
                       $date = strtotime(date('Y-m-d h:i:s')).$check['id'];
                       $selfie="upload/selfie3/$date.jpg";
                ...
                $res = $this->app->insertAttendance($data);
                if($res){
                  $notificationTitle = "Punch IN/Out";
                  $notificationMessage = $check['name']." Punch ".$at->mode." Done at ".date("h:s a",$at->time);
                  ...
                  $res=$this->db->insert('notifications',$notificationData);
                  $message = [
                    'token' => $company['firebassid'],
                    'notification' => [
                        'title' => $notificationTitle,
      ...
      $offline = $this->app->getUserAttendanceAll($check['id'],$offlineTime);
      $newOffline = array();
      ...
      $sendRes = array('msg'=>$msg,'status'=>$status,'offline'=>$newOffline,'logout'=>'0');
  ...
  function getUserTasks(){
    ...
    if(isset($data->checkon)){
      ...
      $getNotifications = $this->app->getNotifications($check['id']);
      $rules = array();
      ...
      if(true){
        ...
        $users_data = $this->app->getCompanyUsers($userCmp['business_id']);
        
        foreach($users_data as $user){
            if(($user->doj!="" || time()>=$user->doj) && ($user->left_date=="" || time()<$user->left_date)){
                $selfie = $user->image;
              if(!empty($selfie)){
                  $url=base_url();
                  $selfie=$url.'/'.$selfie;
        ...
        $punchMode = $this->app->updatePunchMode($userCmp['business_id'],$check['id'],$data->checkon->punch_mode);     
        $userCmp = $this->app->getUserCompany($check['id']);
        $company = $this->app->getbussnames($userCmp['business_id']);
        $user_status = $this->app->userCmpStatus($check['id'],$userCmp['business_id']);
        ...
        $tasks = $this->app->getUserTask($userCmp['business_id'],$check['id']);
        $rules = $this->app->getAttendanceRules($userCmp['business_id']);
        $clients = $this->app->getUserClients($userCmp['business_id'],$check['id']);
        $options = array(
          'qr'=>$userCmp['qr'],
          'gps'=>$userCmp['gps'],
          'face'=>$userCmp['face'],
          ...
          'punch_mode'=>$userCmp['punch_mode'],
          'qr_mode'=>$userCmp['qr_mode'],
          'hostel'=>$userCmp['hostel'],
          'rule_id'=>$userCmp['rule_id'],
          'month_weekly_off'=>$userCmp['month_weekly_off']
        );
        ...
        $cmp_array = array(
          ...
          'newQr'=>$newQrs,
          'manager'=>$check['manager'],
          'business_group'=>$check['business_group']
        );
        $holidays = $this->app->getHoliday($userCmp['business_id']);
        $groups = $this->app->getUserGroup($check['business_group']);
    ...
    //   }
      ...
      $sendRes = array('msg'=>$msg,'status'=>$status,'offline'=>$newOffline,'company_data'=>$cmp_array,'holiday'=>$holidays,'group'=>$groups,'leaves'=>$leaves,'logout'=>'0','sections'=>$sections,'user_section'=>$check['section'],'options'=>$options,'user_images'=>$users_images,'task'=>$tasks,'rules'=>$rules,'notifications'=>$getNotifications,'clients'=>$clients);
      $response= json_encode(array('checkon'=>$sendRes));
      echo $response;
    }else{
        $cmp_array = array('company_name'=>"",'company_image'=>"",'status'=>'0','preMonth'=>0,'nextMonth'=>0,'month'=>'0','manager'=>'0','logout'=>'0');
  ...
  function updateSection(){
    ...
    if(key($data)=="checkon"){
      ...
      if(!empty($check['id']) && $check['user_group']==1 && !empty($cmpMac)){
        $res = $this->app->updateCompanyMacByType($check['id'],$data->checkon->name,$data->checkon->ssid,$data->checkon->mac,$data->checkon->strength,$data->checkon->type,$data->checkon->location,$data->checkon->latitude,$data->checkon->longitude,$data->checkon->radius,$data->checkon->bluetooth_name,$data->checkon->bluetooth_mac,$data->checkon->bluetooth_strength);
        if($res){
          $cmpMac = $this->app->getCompanyMacByType($check['id'],$data->checkon->type);
          $sendRes = array('msg'=>'Wifi Added','status'=>'1','data'=>$cmpMac);
          echo $response= json_encode(array('checkon'=>$sendRes));
        ...
        'bluetooth_name'=>$data->checkon->bluetooth_name,
        'bluetooth_mac'=>$data->checkon->bluetooth_mac,
        'bluetooth_strength'=>$data->checkon->bluetooth_strength,
        'date_time'=>time()
        );
  ...
  function updateEmpOptions(){
   ...
   if(key($data)=="checkon"){
     ...
     if(!empty($check['id']) && $check['user_group']==1){
       ...
       if($dt){
         $res = $this->app->updateCmpOptions($check['id'],$data->checkon->auto_gps,$data->checkon->qr,$data->checkon->gps,$data->checkon->field_duty,$data->checkon->four_layer_security,$data->checkon->face,$data->checkon->selfie_with_gps,$data->checkon->selfie_with_field_duty,$data->checkon->gps_tracking,$data->checkon->qr_mode);
       }else{
         $options=array(
           'bid'=>$check['id'],
           'qr'=>$data->checkon->qr,
           ...
           'gps_tracking'=>$data->checkon->gps_tracking,
           'qr_mode'=>$data->checkon->qr_mode,
           'date_time'=>time()
         );
         $res = $this->app->insertCmpOptions($options);
       }
       $res = $this->app->updateEmpOptions($check['id'],$data->checkon->auto_gps,$data->checkon->qr,$data->checkon->gps,$data->checkon->field_duty,$data->checkon->four_layer_security,$data->checkon->face,$data->checkon->selfie_with_gps,$data->checkon->selfie_with_field_duty,$data->checkon->gps_tracking,$data->checkon->qr_mode);
       if($res){
  ...
  function getEmpLeavesType(){
   ...
   if(key($data)=="checkon"){
     ...
     if(!empty($check['id']) && $check['user_group']==2){
       $userCmp = $this->app->getUserCompany($check['id']);
       if(isset($userCmp) && ($userCmp['left_date']=="" || $userCmp['left_date']>time())){
         $res = $this->app->getOpenLeave($userCmp['business_id'],$check['id'],time());
         if(!empty($res)){
          $yearName = date('Y');
		  $monthName = date('m');
          $maxDays = date("t",strtotime($yearName."-".$monthName."-01"));
		  $monthStartTime = strtotime(date("d-m-Y 00:00:00",strtotime($yearName."-".$monthName."-01"))." +1 days");
		  $monthEndTime = strtotime(date("d-m-Y 23:59:59",strtotime($yearName."-".$monthName."-01"))." +".$maxDays." days");

          $leaves = $this->app->getMonthLeaves($userCmp['business_id'],$check['id'],$monthStartTime,$monthEndTime);
          
          foreach($leaves as $leave){
            if($leave->status==2 || $leave->status==1){
              if($leave->type=="cl" && $res['cl']>0){
                $res['cl']--;
              }else if($leave->type=="pl" && $res['pl']>0){
                $res['pl']--;
              }else if($leave->type=="el" && $res['el']>0){
                $res['el']--;
              }else if($leave->type=="sl" && $res['sl']>0){
                $res['sl']--;
              }else if($leave->type=="hl" && $res['hl']>0){
                $res['hl']--;
              }else if($leave->type=="rh" && $res['rh']>0){
                $res['rh']--;
              }else if($leave->type=="comp_off" && $res['comp_off']>0){
                $res['comp_off']--;
              }else if($leave->type=="other" && $res['other']>0){
                $res['other']--;
              }
            }
          }
  ...
  function manageFaceAttendance(){
    ...
    function getCompanyLicence(){
     ...
     if(key($data)=="checkon"){
       ...
       if(!empty($check['id'])){
         ...
         $newQr=$this->app->getCompanyAllLicence($check['id']);
         if(!empty($newQr)){
             foreach($newQr as $qr){
    ...
    function transferLicence(){
     ...
     function getCompanyRoleUsers(){
      $data=json_decode(file_get_contents("php://input"));
      if(key($data)=="checkon"){
        $check=$this->app->checkMobile($data->checkon->mobile);
        if(!empty($check['id']) && $check['user_group']==1){
          $users = $this->app->getCompanyRoleUsers($check['id']);
            echo $response= json_encode(array('checkon'=>$users));
          }else{
            $sendRes = array('msg'=>'No Data Found','status'=>'0');
            echo $response= json_encode(array('checkon'=>$sendRes));
        }
     ...
     function addUserRole(){
      ...
      if(key($data)=="checkon"){
        $check=$this->app->checkMobile($data->checkon->mobile);
        if(!empty($check['id']) && $check['user_group']==1){
          $checkUser=$this->app->checkMobile($data->checkon->userMobile);
          if(!empty($checkUser['id'])){
            ...
            $addUserRole = $this->app->addUserRole($data);
            $sendRes = array('msg'=>'User Added Successfully','status'=>'1');
            echo $response= json_encode(array('checkon'=>$sendRes));
          }else{
            $sendRes = array('msg'=>'User Already Exists','status'=>'1');
            echo $response= json_encode(array('checkon'=>$sendRes));
          }          
          }else{
            $sendRes = array('msg'=>'No Data Found','status'=>'0');
     ...
     function changeUserRoleStatus(){
      $data=json_decode(file_get_contents("php://input"));
      if(key($data)=="checkon"){
        $check=$this->app->checkMobile($data->checkon->mobile);
        if(!empty($check['id']) && $check['user_group']==1){
          $res=$this->app->changeUserRoleStatus($check['id'],$data->checkon->userId,$data->checkon->status);
          if($res){
            $sendRes = array('msg'=>'Status Changed','status'=>'1');
            echo $response= json_encode(array('checkon'=>$sendRes));
          }else{
            $sendRes = array('msg'=>'Unable to Update','status'=>'0');
            echo $response= json_encode(array('checkon'=>$sendRes));
          }
          }else{
            $sendRes = array('msg'=>'No Data Found','status'=>'0');
    ...
    function checkUserRoleStatus(){
      ...
      if(key($data)=="checkon"){
        $check=$this->app->checkMobile($data->checkon->mobile);
        if(!empty($check['id'])){
          $res=$this->app->checkUserRoleStatus($check['id'],$data->checkon->userId);
          $linkedAdmin = $this->app->getAllAdmins($check['id']);
          if(!empty($res) && ($res['status']==0 || $res['deleted']==1)){
            $sendRes = array('msg'=>'Stay','status'=>'1','linkedAdmin'=>$linkedAdmin);
            echo $response= json_encode(array('checkon'=>$sendRes));
          }else{
            $sendRes = array('msg'=>'Login','status'=>'0','linkedAdmin'=>$linkedAdmin);
            echo $response= json_encode(array('checkon'=>$sendRes));
          }
          }else{
            $sendRes = array('msg'=>'No Data Found','status'=>'0','linkedAdmin'=>$linkedAdmin);
            echo $response= json_encode(array('checkon'=>$sendRes));
        }
      }
     }
     ...
     function deleteUserRole(){
      ...
      if(key($data)=="checkon"){
        $check=$this->app->checkMobile($data->checkon->mobile);
        if(!empty($check['id']) && $check['user_group']==1){
          $res=$this->app->deleteUserRole($check['id'],$data->checkon->userId);
          if($res){
            $sendRes = array('msg'=>'Deleted Successfully','status'=>'1');
            echo $response= json_encode(array('checkon'=>$sendRes));
          }else{
            $sendRes = array('msg'=>'Unable to Delete','status'=>'0');
            echo $response= json_encode(array('checkon'=>$sendRes));
          }
          }else{
            $sendRes = array('msg'=>'No Data Found','status'=>'0');
     ...
     function addGpsTracking(){
      $data=json_decode(file_get_contents("php://input"));
      if(key($data)=="checkon"){
        $list = $data->checkon->offline;
        $gpsLocal = $data->checkon->gpsLocal;
        $offlineTime = $data->checkon->offlineTime;
        $check=$this->app->checkMobile($data->checkon->mobile);
        $userCmp = $this->app->getUserCompany($check['id']);
        if($check['device_id']==$data->checkon->device_id){
        ...
        foreach($list as $at){
          if(!empty($check['id']) && $check['user_group']==2){
            $user_status = $this->app->userCmpStatus($check['id'],$at->bid);
            if( !empty($userCmp['business_id']) && $userCmp['business_id']==$at->bid){
              if($user_status['user_status']=="1"){
                $checkOffline = $this->app->checkGpsTrackingTime($check['id'],$at->bid,$at->time);
                if(empty($checkOffline)){
                  $data = array(
                  'bid'=>$at->bid,
                  'uid'=>$check['id'],
                  ...
                  'longitude'=>$at->longitude,
                  'location'=>$at->location,
                  'exit_time'=>$at->exit_time,
                  'date_time'=>time()
                  );
                  $res = $this->app->insertGpsTracking($data);
                  $msg = 'Added Offline Gps Tracking';
                  $status= '1';
                }else{
                    $this->app->updateGpsTracking($checkOffline['id'],$at->exit_time);
                    $msg = 'Added Offline Gps Tracking';
                    $status= '1';
                }
              }else{
        ...
        foreach($gpsLocal as $at){
          if(!empty($check['id']) && $check['user_group']==2){
            $user_status = $this->app->userCmpStatus($check['id'],$at->bid);
            if( !empty($userCmp['business_id']) && $userCmp['business_id']==$at->bid){
              if($user_status['user_status']=="1"){
                $checkOffline = $this->app->checkGpsLocationTime($check['id'],$at->bid,$at->time);
                if(empty($checkOffline)){
                  $data = array(
                  'bid'=>$at->bid,
                  'uid'=>$check['id'],
                  ...
                  $res = $this->app->insertGpsLocation($data);
                  $msg = 'Added Offline Gps Tracking';
                  $status= '1';
                }
              }else{
        ...
        $newOffline = $this->app->getUserGpsTrackingAll($check['id'],$offlineTime);

        $newGpsOffline = $this->app->getUserGpsLocationAll($check['id'],$offlineTime);
  
        $sendRes = array('msg'=>$msg,'status'=>$status,'offline'=>$newOffline,'gpsLocation'=>$newGpsOffline);
        $response= json_encode(array('checkon'=>$sendRes));
        echo $response;
      }else{
          $msg = 'Please Add Company';
  ...
  function getCompanyGpsTrackingUsers(){
    $data=json_decode(file_get_contents("php://input"));
    if(key($data)=="checkon"){
      $check=$this->app->checkMobile($data->checkon->mobile);
       if($check['validity']!="" && $check['validity']>time()){
      ...
      if(!empty($check['id']) && $check['user_group']==1 && $check['prime_att']==1){
        ...
        if(!empty($users_data)){
          ...
          foreach($users_data as $user){
              if(($user->doj!="" || $start_time>=$user->doj) && ($user->left_date=="" || $start_time<$user->left_date)){
                $user_at = $this->app->getUserGpsTrackingByDate($start_time,$end_time,$user->user_id,$check['id']);
                $data = array();
  ...
  function getCompanyGpsLocationUsers(){
    $data=json_decode(file_get_contents("php://input"));
    if(key($data)=="checkon"){
      $check=$this->app->checkMobile($data->checkon->mobile);
       if($check['validity']!="" && $check['validity']>time()){
      ...
      if(!empty($check['id']) && $check['user_group']==1 && $check['prime_att']==1){
        ...
        if(!empty($users_data)){
          ...
          foreach($users_data as $user){
              if(($user->doj!="" || $start_time>=$user->doj) && ($user->left_date=="" || $start_time<$user->left_date)){
                $user_at = $this->app->getUserGpsLocationByDate($start_time,$end_time,$user->user_id,$check['id']);
                $data = array();
                ...
                $gpsTracking = $this->app->getUserGpsTrackingByDate($start_time,$end_time,$user->user_id,$check['id']);
                $gpsTrackingOffline = array();
                foreach($gpsTracking as $at){
                    $gpsTrackingOffline[]=array(
                      "id"=>$at->id,
                      "bid"=>$at->bid,
                      "uid"=>$at->uid,
                ...
                $new_array[] =array(
                  ...
                  'user_status'=>$user->user_status,
                  'data'=> $data,
                  'gpsTracking'=>$gpsTrackingOffline
                );
              }
          }
          echo $response= json_encode(array('checkon'=>$new_array));
  ...
  function getGpsTrackingByUserId(){
    $data=json_decode(file_get_contents("php://input"));
    if(key($data)=="checkon"){
      $check=$this->app->checkMobile($data->checkon->mobile);
       if($check['validity']!="" && $check['validity']>time()){
      ...
      if(!empty($check['id']) && $check['user_group']==1 && $check['prime_att']==1){
        ...
        if(!empty($users_data)){
          $offline = $this->app->getUserGpsTrackingByDate($start_time,$end_time,$data->checkon->empId,$check['id']);
          $newOffline = array();
          foreach($offline as $at){
            $newOffline[]=array(
              "id"=>$at->id,
            ...
            $user_at = $this->app->getUserGpsLocationByDate($start_time,$end_time,$data->checkon->empId,$check['id']);
            $data = array();
          ...
          echo $response= json_encode(array('checkon'=>$newOffline,'local'=>$data));
        }else{
          $res=array('msg'=>'No Data Foud');
          echo $response= json_encode(array('checkon'=>$res));
        }
    ...
    if(key($data)=="checkon"){
      ...
      if(!empty($check['id']) && $check['user_group']==2){
        $userCmp = $this->app->getUserCompany($check['id']);
          ...
          $res = $this->app->insertAttendance($data);
          if($res == 1){
            $sendRes = array('msg'=>'Attendance Requested Successfully','status'=>'1');
          }else{
            $sendRes = array('msg'=>'Failed to Request','status'=>'0');
          }
          echo $response= json_encode(array('checkon'=>$sendRes));
  ...
  function getCompanyUsersOffline(){
    $data=json_decode(file_get_contents("php://input"));
    if(key($data)=="checkon"){
      $check=$this->app->checkMobile($data->checkon->mobile);
       if($check['validity']!="" && $check['validity']>time()){
      ...
      if(!empty($check['id']) && $check['user_group']==1 && $check['prime_att']==1){
        $users_data = $this->app->getCompanyUserRequest($check['id']);
        $start_time = strtotime(date("d-m-Y 00:00:00",strtotime($data->checkon->date)));
        $end_time = strtotime(date("d-m-Y 23:59:59",strtotime($data->checkon->date)));

        $holidays = $this->app->getHoliday($check['id']);
        $groups = $this->app->getBusinessGroups($check['id']);
        $sections = $this->app->getCompanyAllMac($check['id']);
        $tasks = $this->app->getCompanyTask($check['id']);
        $rules = $this->app->getAttendanceRules($check['id']);
        $getCmpOptions = $this->app->getCmpOptions($check['id']);
        $getDepartmentSections = $this->app->getDepartmentSections($check['id']);
        $getCmpSalaryReport = $this->app->getCmpSalaryReport($check['id']);
        $getCmpPayrollHistory = $this->app->getCmpPayrollHistory($check['id']);
        $getCmpUserCtc = $this->app->getCmpUserCtc($check['id']);
        $getCmpPayrollMaster = $this->app->getCmpPayrollMaster();
        $getNotifications = $this->app->getNotifications($check['id']);
        if(true){
          $new_array = array();
          $dayAt = $this->app->getCompanyAttendanceByDate($check['id'],$start_time,$end_time);
          foreach($users_data as $user){
            $uid =$user->user_id;
            $offline_user = $this->app->getCompanyUser($user->user_id);
            //$offline_at = $this->app->getCompanyAttendanceByDate($user->user_id,$check['id'],$start_time,$end_time);
            $offline_at = array_filter($dayAt, function($val) use($uid){
            ...
            $user_image = $offline_user['image'];
            if(!empty($user_image)){
                $url=base_url();
                $user_image=$url.'/'.$user_image;
                $im = file_get_contents($user_image);
                $user_image = base64_encode($im);
            }
            
            $offline_user['image'] = $user_image;
            $offline_user['data'] = $newOffline;
            $offline_user['leaves'] = $leaves;
            if(isset($offline_user['id'])){
                $new_array[] = $offline_user;
            }
            
          }
          echo $response= json_encode(array('checkon'=>$new_array,'groups'=>$groups,'holidays'=>$holidays,'user_request'=>$users_data,'prime'=>'1','sections'=>$sections,'tasks'=>$tasks,'rules'=>$rules,'cmpOptions'=>$getCmpOptions,'departmentSection'=>$getDepartmentSections,'salaryReport'=>$getCmpSalaryReport,'payrollHistory'=>$getCmpPayrollHistory,'userCtc'=>$getCmpUserCtc,'payrollMaster'=>$getCmpPayrollMaster,'notifications'=>$getNotifications));
        }else{
          $res=array('msg'=>'No Data Foud');
          echo $response= json_encode(array('checkon'=>$res));
        }
          ...
          $res=array('msg'=>'Currently Inactive','prime'=>'0');
          echo $response= json_encode(array('checkon'=>$res,'prime'=>'0'));
      }
    }
  }
  ...
  function getTask(){
    ...
    if(key($data)=="checkon"){
      ...
      if(!empty($check['id']) && $check['user_group']==2){
        $userCmp = $this->app->getUserCompany($check['id']);
        if(!empty($userCmp)){
          $getUserTask = $this->app->getUserTask($userCmp['business_id'],$check['id']);
          echo $response= json_encode(array('checkon'=>$getUserTask));
        }
      }
    }
  }
  ...
  function addUserTask(){
    ...
    if(key($data)=="checkon"){
      ...
      if(!empty($check['id']) && $check['user_group']==2){
        $userCmp = $this->app->getUserCompany($check['id']);
        if(!empty($userCmp)){
            $getCompanyUser = $this->app->getCompanyUser($userCmp['business_id']);
            print_r($getCompanyUser);
            if(!empty($getCompanyUser)){
                $notificationTitle = "Task Added";
                $notificationMessage = "A new task has been added by ".$check['name'];
                $notificationData=array(
                ...
                $res=$this->db->insert('notifications',$notificationData);
              $message = [
                'token' => $getCompanyUser['firebassid'],
                'notification' => [
                    'title' => $notificationTitle,
                    'body' => $notificationMessage,
                ],
          ...
          $data = array(
            'bid'=>$userCmp['business_id'],
            'uid'=>$check['id'],
            'group_id'=>$data->checkon->groupId,
            'assign_id'=>$check['id'],
            'task'=>$data->checkon->task,
            'priority'=>$data->checkon->priority,
            'status'=>$data->checkon->status,
            'client_name'=>$data->checkon->client_name,
            'client_mobile'=>$data->checkon->client_mobile,
            'client_address'=>$data->checkon->client_address,
            'time'=>$data->checkon->time,
            'amount'=>$data->checkon->amount,
            'date_time'=>time()
          );
          $res = $this->app->addTask($data);
          if($res == 1){
            $sendRes = array('msg'=>'Task Added Successfully','status'=>'1');
          }else{
            $sendRes = array('msg'=>'Failed to Add Task','status'=>'0');
          }
          echo $response= json_encode(array('checkon'=>$sendRes));
        }
      }
  ...
  function updateUserTask(){
    ...
    if(key($data)=="checkon"){
      ...
      if(!empty($check['id']) && $check['user_group']==2){
        $userCmp = $this->app->getUserCompany($check['id']);
        if(!empty($userCmp)){
          $getUserTaskById = $this->app->getUserTaskById($data->checkon->id);
          if(!empty($getUserTaskById)){
            $getCompanyUser = $this->app->getCompanyUser($userCmp['business_id']);
            if(!empty($getCompanyUser)){
                if($data->checkon->deleted==0){
                    $notificationTitle = "Task Updated";
                    $notificationMessage = "A task has been updated by ".$check['name'];
                }else{
                    $notificationTitle = "Task Deleted";
                    $notificationMessage = "A task has been deleted by ".$check['name'];
                }
                $notificationData=array(
                  'uid'=>$userCmp['business_id'],
                  'title'=>$notificationTitle,
          ...
          $updateData = array(
            'group_id'=>$data->checkon->groupId,
            'assign_id'=>$data->checkon->assign_id,
            'task'=>$data->checkon->task,
            'priority'=>$data->checkon->priority,
            'status'=>$data->checkon->status,
            'client_name'=>$data->checkon->client_name,
            ...
            'time'=>$data->checkon->time,
            'amount'=>$data->checkon->amount,
            'deleted'=>$data->checkon->deleted
          );
          $res = $this->app->updateUserTask($data->checkon->id,$updateData);
          if($res == 1){
            $sendRes = array('msg'=>'Task Updated Successfully','status'=>'1');
          }else{
            $sendRes = array('msg'=>'Failed to Update Task','status'=>'0');
          }
          echo $response= json_encode(array('checkon'=>$sendRes));
        }
      }
  ...
  function getCompanyTask(){
    ...
    if(key($data)=="checkon"){
      ...
      if(!empty($check['id']) && $check['user_group']==1 && $check['prime_att']==1){
        $tasks = $this->app->getCompanyTask($check['id']);
        $leaves = $this->app->getCompanyLeaves($check['id']);
        echo $response= json_encode(array('checkon'=>$tasks,'leaves'=>$leaves));
      }else{
          $res=array('msg'=>'Currently Inactive','prime'=>'0');
          echo $response= json_encode(array('checkon'=>$res,'prime'=>'0'));
      }
  ...
  function updateCompanyTask(){
    $data=json_decode(file_get_contents("php://input"));
    if(key($data)=="checkon"){
      $check=$this->app->checkMobile($data->checkon->mobile);
      if(!empty($check['id']) && $check['user_group']==1){
          $getUserTaskById = $this->app->getUserTaskById($data->checkon->id);
          if(!empty($getUserTaskById) && $data->checkon->assign_id!=$check['id']){
            $getCompanyUser = $this->app->getCompanyUser($data->checkon->assign_id);
            if(!empty($getCompanyUser)){
              if($getUserTaskById['assign_id']!=$data->checkon->assign_id){
                $notificationTitle = "Task Assigned";
                $notificationMessage = "A new task has been assigned to you";
  ...
  function getCompanyLeaves(){
    ...
    if(key($data)=="checkon"){
      ...
      if(!empty($check['id']) && $check['user_group']==1 && $check['prime_att']==1){
        $leaves = $this->app->getCompanyLeaves($check['id']);
        echo $response= json_encode(array('checkon'=>$leaves));
      }else{
          $res=array('msg'=>'Currently Inactive','prime'=>'0');
          echo $response= json_encode(array('checkon'=>$res,'prime'=>'0'));
      }
  ...
  function updateCompanyLeave(){
    $data=json_decode(file_get_contents("php://input"));
    if(key($data)=="checkon"){
      $check=$this->app->checkMobile($data->checkon->mobile);
      if(!empty($check['id']) && $check['user_group']==1){
          $from_date= strtotime(date("d-m-Y 00:00:00",strtotime($data->checkon->fromDate)));
          $to_date = strtotime(date("d-m-Y 00:00:00",strtotime($data->checkon->toDate)));
          $updateData = array(
            'status'=>$data->checkon->status,
            'from_date'=>$from_date,
            'to_date'=>$to_date
          );
          $res = $this->app->updateCompanyLeave($data->checkon->id,$updateData);
          if($res == 1){
            $sendRes = array('msg'=>'Leave Updated Successfully','status'=>'1');
          }else{
            $sendRes = array('msg'=>'Failed to Update Leave','status'=>'0');
          }
          echo $response= json_encode(array('checkon'=>$sendRes));
      }
    }
      ...
      if(!empty($check['id']) && $check['user_group']==2){
        ...
        if(!empty($userCmp)){
          $data = array(
            'bid'=>$userCmp['business_id'],
            'uid'=>$check['id'],
            'category'=>$data->checkon->category,
            'complain'=>$data->checkon->complain,
            'status'=>$data->checkon->status,
            'date_time'=>time()
          );
          $res = $this->app->addComplain($data);
          if($res == 1){
            $sendRes = array('msg'=>'Complain Added Successfully','status'=>'1');
          }else{
            $sendRes = array('msg'=>'Failed to Add Complain','status'=>'0');
          }
          echo $response= json_encode(array('checkon'=>$sendRes));
        }
      }
  ...
  function getUserComplain(){
    ...
    if(key($data)=="checkon"){
      ...
      if(!empty($check['id']) && $check['user_group']==2){
        $userCmp = $this->app->getUserCompany($check['id']);
        if(!empty($userCmp)){
          $getUserTask = $this->app->getUserComplain($userCmp['business_id'],$check['id']);
          echo $response= json_encode(array('checkon'=>$getUserTask));
        }
      }
    }
  ...
  function getCompanyComplains(){
    ...
    if(key($data)=="checkon"){
      ...
      if(!empty($check['id']) && $check['user_group']==1 && $check['prime_att']==1){
        $complains = $this->app->getCompanyComplains($check['id']);
        echo $response= json_encode(array('checkon'=>$complains));
      }else{
          $res=array('msg'=>'Currently Inactive','prime'=>'0');
          echo $response= json_encode(array('checkon'=>$res,'prime'=>'0'));
      }
  ...
  function updateCompanyComplain(){
    $data=json_decode(file_get_contents("php://input"));
    if(key($data)=="checkon"){
      $check=$this->app->checkMobile($data->checkon->mobile);
      if(!empty($check['id']) && $check['user_group']==1){
          ...
          $res = $this->app->updateUserComplain($data->checkon->id,$updateData);
          if($res == 1){
            $sendRes = array('msg'=>'Complain Updated Successfully','status'=>'1');
          }else{
            $sendRes = array('msg'=>'Failed to Update Complain','status'=>'0');
          }
          echo $response= json_encode(array('checkon'=>$sendRes));
      }
    }
  ...
  function addCompanyBlock(){
    ...
    if(key($data)=="checkon"){
      ...
      if(!empty($check['id']) && $check['user_group']==1){
          ...
          $res = $this->app->addCompanyBlock($data);
          if($res == 1){
            $sendRes = array('msg'=>'Block Added Successfully','status'=>'1');
          }else{
            $sendRes = array('msg'=>'Failed to Add Block','status'=>'0');
          }
          echo $response= json_encode(array('checkon'=>$sendRes));
      }
    }
  ...
  function updateCompanyBlock(){
    ...
    if(key($data)=="checkon"){
      ...
      if(!empty($check['id']) && $check['user_group']==1){
          ...
          $res = $this->app->updateCompanyBlock($data->checkon->id,$updateData);
          if($res == 1){
            $sendRes = array('msg'=>'Block Updated Successfully','status'=>'1');
          }else{
            $sendRes = array('msg'=>'Failed to Update Block','status'=>'0');
          }
          echo $response= json_encode(array('checkon'=>$sendRes));
      }
    }
  ...
  function getCompanyBlocks(){
    ...
    if(key($data)=="checkon"){
      ...
      if(!empty($check['id']) && $check['user_group']==1 && $check['prime_att']==1){
        $blocks = $this->app->getCompanyBlocks($check['id']);
        echo $response= json_encode(array('checkon'=>$blocks));
      }else{
          $res=array('msg'=>'Currently Inactive','prime'=>'0');
          echo $response= json_encode(array('checkon'=>$res,'prime'=>'0'));
      }
  ...
  function getCompanyRoomType(){
    ...
    if(key($data)=="checkon"){
      ...
      if(!empty($check['id']) && $check['user_group']==1 && $check['prime_att']==1){
        $blocks = $this->app->getCompanyRoomType($check['id']);
        echo $response= json_encode(array('checkon'=>$blocks));
      }else{
          $res=array('msg'=>'Currently Inactive','prime'=>'0');
          echo $response= json_encode(array('checkon'=>$res,'prime'=>'0'));
  ...
  function addCompanyRoomType(){
    ...
    if(key($data)=="checkon"){
      ...
      if(!empty($check['id']) && $check['user_group']==1){
          ...
          $res = $this->app->addCompanyRoomType($data);
          if($res == 1){
            $sendRes = array('msg'=>'Room Type Added Successfully','status'=>'1');
          }else{
            $sendRes = array('msg'=>'Failed to Add Room Type','status'=>'0');
          }
          echo $response= json_encode(array('checkon'=>$sendRes));
      }
    }
  ...
  function updateCompanyRoomType(){
    $data=json_decode(file_get_contents("php://input"));
    if(key($data)=="checkon"){
      $check=$this->app->checkMobile($data->checkon->mobile);
      if(!empty($check['id']) && $check['user_group']==1){
          ...
          $res = $this->app->updateCompanyRoomType($data->checkon->id,$updateData);
          if($res == 1){
            $sendRes = array('msg'=>'Block Updated Successfully','status'=>'1');
          }else{
            $sendRes = array('msg'=>'Failed to Update Block','status'=>'0');
  ...
  function getHostelEmpProfile(){
    $data=json_decode(file_get_contents("php://input"));
    if(key($data)=="checkon"){
      $check=$this->app->checkMobile($data->checkon->mobile);
      if(!empty($check['id']) && $check['user_group']==1){
        $emp = $this->app->getHostelEmpProfile($data->checkon->uid);
        $blocks = $this->app->getCompanyBlocks($check['id']);
        $roomTypes = $this->app->getCompanyRoomType($check['id']);
        if(!empty($emp)){
          $data = array(
            'user'=>$emp,
            'blocks'=>$blocks,
  ...
  function updateHostelEmpProfile(){
    $data=json_decode(file_get_contents("php://input"));
    if(key($data)=="checkon"){
      $check=$this->app->checkMobile($data->checkon->mobile);
      if(!empty($check['id']) && $check['user_group']==1){
          $profile = array(
            ...
            "address" => $data->checkon->address,
            "gender" => $data->checkon->gender,
            "bio_id" => $data->checkon->device_id
          );
          $res = $this->app->updateHostelEmpProfile($data->checkon->uid,$profile);
          $hostelDetail = $this->app->getHostelDetails($check['id'],$data->checkon->uid);
          
          $hostelData = array(
            "bid" => $check['id'],
            "uid" => $data->checkon->uid,
            "block" => $data->checkon->block,
            "room_no" => $data->checkon->room_no,
            "floor" => $data->checkon->floor,
            "room_type" => $data->checkon->room_type,
            "parent_name" => $data->checkon->parent_name,
            "parent_relation" => $data->checkon->parent_relation,
            "parent_mobile" => $data->checkon->parent_mobile,
            "date_time"=> time()
          );
          if(empty($hostelDetail)){
            $res = $this->app->addHostelDetail($hostelData);
          }else{
            $res = $this->app->updateHostelDetail($hostelDetail['id'],$hostelData);
          }
          if($res == 1){
            $sendRes = array('msg'=>'Profile Updated Successfully','status'=>'1');
          }else{
            $sendRes = array('msg'=>'Failed to Update Profile','status'=>'0');
          }
          echo $response= json_encode(array('checkon'=>$sendRes));
      }
    }
 ...
 function getHostelUsersOffline(){
    $data=json_decode(file_get_contents("php://input"));
    if(key($data)=="checkon"){
      $check=$this->app->checkMobile($data->checkon->mobile);
       if($check['validity']!="" && $check['validity']>time()){
      ...
      if(!empty($check['id']) && $check['user_group']==1 && $check['prime_att']==1){
        $users_data = $this->app->getHostelUserRequest($check['id']);
        $start_time = strtotime(date("d-m-Y 00:00:00",strtotime($data->checkon->date)));
        $end_time = strtotime(date("d-m-Y 23:59:59",strtotime($data->checkon->date)));
      ...
      //  $rules = $this->app->getAttendanceRules($check['id']);
        if(!empty($users_data)){
          $new_array = array();
          $dayAt = $this->app->getHostelAttendanceByDate($check['id'],$start_time,$end_time);
          foreach($users_data as $user){
            $uid =$user->user_id;
            $offline_user = $this->app->getCompanyUser($user->user_id);
            //$offline_at = $this->app->getCompanyAttendanceByDate($user->user_id,$check['id'],$start_time,$end_time);
          ...
          echo $response= json_encode(array('checkon'=>$new_array,'user_request'=>$users_data,'prime'=>'1'));
        }else{
          $res=array('msg'=>'No Data Foud');
          echo $response= json_encode(array('checkon'=>$res));
        }
  ...
  function addHostel(){
   ...
   if(key($data)=="checkon"){
     $check=$this->app->checkMobile($data->checkon->mobile);
     if(!empty($check['id']) && $check['user_group']==2){
      $userCmp = $this->app->getUserHostel($check['id']);
      ...
      if(empty($userCmp) || ($userCmp['left_date']=="" || $userCmp['left_date']<time())){
         ...
         if($cmp_group==1){
           ...
           $uCheck = $this->app->checkStudentAlready($loginid,$check['id']);
           if(count($uCheck)==0){
              $req = $this->app->add_Student_hostel($data);
              if($req){
                 //$res = $this->app->updateUserCompany($check['id'],$loginid,strtotime(date('d-m-Y')));
               ...
               //$res = $this->app->updateUserCompany($check['id'],$loginid,strtotime(date('d-m-Y')));
               //$res = $this->app->updateUserDoj($check['id'],$loginid,'',strtotime(date('d-m-Y')));
               $sendRes = array('msg'=>'Company already Added','status'=>'1','company_id'=>$loginid);
           echo $response= json_encode(array('checkon'=>$sendRes));
           }
 ...
 function addStudentAttendance(){
   ...
   if(key($data)=="checkon"){
     ...
     if(!empty($check['id']) && $check['user_group']==2){
       ...
       $user_status = $this->app->studentHostelStatus($check['id'],$loginid);
       ...
       if( !empty($userCmp['business_id']) && $userCmp['business_id']==$loginid){
         if($user_status['user_status']=="1"){
           ...
           $res = $this->app->insert_Hostel_Attendance($data);
           if($res == 1){
             $sendRes = array('msg'=>'Attendance added Successfully','status'=>'1');
           }else{
             $sendRes = array('msg'=>'Failed to Add','status'=>'0');
 ...
 function childReport(){
  $data=json_decode(file_get_contents("php://input"));
  if(key($data)=="checkon"){
    $child=$this->app->getChild($data->checkon->mobile);
    if(isset($child['mobile'])){
    $check=$this->app->checkMobile($child['mobile']);
    $groups = $this->app->getUserGroup($check['business_group']);
    $currentMonth = strtotime(date("d-m-Y",strtotime("0 months")));
    $startTimestamp = strtotime(date("01-m-Y 00:00:00",$currentMonth));
    $endTimestamp = strtotime(date("t-m-Y 12:59:59",$currentMonth));
    ...
    if(true){
      ...
      $msg = 'Response';
      $status= '0';
      $offline = $this->app->getUserAttendanceAll($check['id'],0);
      $newOffline = array();
      foreach($offline as $at){
          $selfie = $at->selfie;
          if(!empty($selfie)){
        ...
        $sendRes = array('msg'=>$msg,'status'=>$status,'offline'=>$newOffline,'company_data'=>$cmp_array,'holiday'=>$holidays,'group'=>$groups,'leaves'=>$leaves,'logout'=>'0','sections'=>$sections,'user_section'=>$check['section'],'options'=>$options,'user_images'=>$users_images,'task'=>$tasks,'rules'=>$rules,'name'=>$check['name'],'mobile'=>$check['mobile']);
        $response= json_encode(array('checkon'=>$sendRes));
        echo $response;
      }else{
          $cmp_array = array('company_name'=>"",'company_image'=>"",'status'=>'0','preMonth'=>0,'nextMonth'=>0,'month'=>'0','manager'=>'0','logout'=>'0');
  ...
  function addStudentInOut(){
    $data=json_decode(file_get_contents("php://input"));
    if(key($data)=="checkon"){
      $check=$this->app->checkMobile($data->checkon->mobile);
      if(!empty($check['id']) && $check['user_group']==1){
 
        $user_status = $this->app->userCmpStatus($data->checkon->student,$check['id']);
 
        $userCmp = $this->app->getUserCompany($data->checkon->student);
        $start_time = strtotime(date("d-m-Y 00:00:00"));
        $end_time = strtotime(date("d-m-Y 23:59:59"));
        
        if( !empty($userCmp['business_id']) && $userCmp['business_id']==$check['id']){
          if($user_status['user_status']=="1"){
            
            $insertData = array(
              'bussiness_id'=>$check['id'],
              'user_id'=>$data->checkon->student,
              'mode'=>$data->checkon->mode,
              'comment'=>$data->checkon->reason,
              'hostel'=>"1",
              'io_time'=>time(),
              'date'=>time()
            );
            $res = $this->app->insertAttendance($insertData);
            $user_at = $this->app->getUserAttendanceByDate($start_time,$end_time,$data->checkon->student,$check['id'],1);
            if($res == 1){
              $sendRes = array('msg'=>'Attendance added Successfully','status'=>'1','data'=>$user_at);
            }else{
              $sendRes = array('msg'=>'Failed to Add','status'=>'0');
            }
            echo $response= json_encode(array('checkon'=>$sendRes));
    ...
    if(key($data)=="checkon"){
      ...
      if(!empty($check['id']) && $check['user_group']==1){
        ...
        if(!empty($addStaff)){
          $userCmp = $this->app->getUserCompany($addStaff['id']);
          if(isset($userCmp) && ($userCmp['left_date']=="" || $userCmp['left_date']>time())){
            $sendRes = array('msg'=>'Already Added in a Hostel','status'=>'2');
          }else{
            $data1=array(
              'doj'=>strtotime(date("d-m-Y 00:00:00",time()))
            );
            ...
            $data=$this->db->insert('user_request',$cmpInData);
            $sendRes = array('msg'=>'New Student Added','status'=>'1');
          }
          echo $response= json_encode(array('checkon'=>$sendRes));
        }else{
          $sendRes = array('msg'=>'No Data Found','status'=>'0');
        ...
        $nmid = $temp_.($str1+1);
         ...
         if($id){
          ...
          $res = $this->app->addHostelDetail($hostelData);
           ...
           echo $response= json_encode(array('checkon'=>array('msg'=>'Student Added','status'=>'1')));
         }else{
           $res=array('msg'=>'Failed to Add Student','status'=>'0');
           echo $response= json_encode(array('checkon'=>$res));
         }
       }else{
         $res=array('msg'=>'Mobile Number Already Registered','status'=>'0');
...
function addRfid(){
    ...
    if(key($data)=="checkon"){
      if($data->checkon->key=="752d2c9ecfe079e5e5f3539f4d750e5c"){
        ...
        $re = $this->app->insertrfidAttendance($rfidData);
        

        $getBusinessByDeviceId = $this->app->getBusinessByDeviceId($loginid);
        
        if(!empty($getBusinessByDeviceId['id'])){
          $studentByRfid = $this->app->getStudentByRfid($uid,$getBusinessByDeviceId['bid']);
          if(!empty($studentByRfid['id'])){
            $ioTime = strtotime($data->checkon->time);
            $start_time = strtotime(date("d-m-Y 00:00:00",$ioTime));
            $end_time = strtotime(date("d-m-Y 23:59:59",$ioTime));
            $offline_at = $this->app->checkOfflineStudentAt($studentByRfid['id'],$getBusinessByDeviceId['bid'],$start_time,$end_time);
            
            //IN==2 , Out==3
            
            $mode = "2";

            if($getBusinessByDeviceId['type']=="1"){
              $mode = "2";
            }else if($getBusinessByDeviceId['type']=="2"){
              $mode = "3";
            }else if($getBusinessByDeviceId['type']=="0"){
              if(!empty($offline_at)){
                if($ioTime-$offline_at['time']>=30*60){
                  if($offline_at['student_status']=="2"){
                    $mode = "3";
                  }else{
                    $mode = "2";
                  }
                }else{
                  $mode = $offline_at['student_status'];
                }
              }
            }
            ...
            $studentData=array(
              'bid'=>$getBusinessByDeviceId['bid'],
              'class_id'=>$studentByRfid['class_id'],
              'student_id'=>$studentByRfid['id'],
              'added_by'=>$getBusinessByDeviceId['id'],
              'student_status'=>$mode,
              'time'=>strtotime($data->checkon->time),
              'update_date'=>time(),
              'date_time'=>time()
            );
            $res=$this->db->insert('student_attendance',$studentData);
            if($res == 1){
              $sendRes = array('msg'=>'Attendance added Successfully','status'=>'1');
            }else{
              $sendRes = array('msg'=>'Failed to Add','status'=>'0');
            ...
            $sendRes = array('msg'=>'Wrong Rfid','status'=>'0');
            echo $response= json_encode(array('checkon'=>$sendRes));
          } 
          echo $response= json_encode(array('checkon'=>$sendRes));
        }else{
          $sendRes = array('msg'=>'Wrong DeviceId','status'=>'0');
          echo $response= json_encode(array('checkon'=>$sendRes));
        }  
      }
    }
   ...
   function getCompanyUsersOfflineNew(){
    $data=json_decode(file_get_contents("php://input"));
    if(key($data)=="checkon"){
      $check=$this->app->checkMobile($data->checkon->mobile);
       if($check['validity']!="" && $check['validity']>time()){
      ...
      if(!empty($check['id']) && $check['user_group']==1 && $check['prime_att']==1){
        ...
        $getCmpPayrollMaster = $this->app->getCmpPayrollMaster();
        $getNotifications = $this->app->getNotifications($check['id']);
        $getCmpClients = $this->app->getCmpClients($check['id']);
        if(true){
          $new_array = array();
          $dayAt = $this->app->getCompanyAttendanceByDate($check['id'],$start_time,$end_time);
          foreach($users_data as $user){
          ...
          echo $response= json_encode(array('checkon'=>$new_array,'groups'=>$groups,'holidays'=>$holidays,'user_request'=>$users_data,'prime'=>'1','sections'=>$sections,'tasks'=>$tasks,'rules'=>$rules,'cmpOptions'=>$getCmpOptions,'departmentSection'=>$getDepartmentSections,'salaryReport'=>$getCmpSalaryReport,'payrollHistory'=>$getCmpPayrollHistory,'userCtc'=>$getCmpUserCtc,'payrollMaster'=>$getCmpPayrollMaster,'notifications'=>$getNotifications,'clients'=>$getCmpClients));
        }else{
          $res=array('msg'=>'No Data Foud');
          echo $response= json_encode(array('checkon'=>$res));
        }
  ...
  function addCompanyClass(){
    ...
    if(key($data)=="checkon"){
      ...
      if(!empty($check['id']) && $check['user_group']==1){
          ...
          $res = $this->app->addCompanyClass($data);
          if($res == 1){
            $sendRes = array('msg'=>'Class Added Successfully','status'=>'1');
          }else{
            $sendRes = array('msg'=>'Failed to Add Class','status'=>'0');
          }
          echo $response= json_encode(array('checkon'=>$sendRes));
      }
    }
  ...
  function updateCompanyClass(){
    ...
    if(key($data)=="checkon"){
      ...
      if(!empty($check['id']) && $check['user_group']==1){
          ...
          $res = $this->app->updateCompanyClass($data->checkon->id,$updateData);
          if($res == 1){
            $sendRes = array('msg'=>'Class Updated Successfully','status'=>'1');
          }else{
            $sendRes = array('msg'=>'Failed to Update Class','status'=>'0');
          }
          echo $response= json_encode(array('checkon'=>$sendRes));
      }
    }
  ...
  function addTeacherToClass(){
    ...
    if(key($data)=="checkon"){
      ...
      if(!empty($check['id']) && $check['user_group']==1){
        $addStaff=$this->app->checkMobile($data->checkon->staffMobile);
        if(!empty($addStaff)){
          $checkTeacher = $this->app->checkTeacher($check['id'],$addStaff['id'],$data->checkon->classId);
          if(!empty($checkTeacher)){
            $sendRes = array('msg'=>'Already Added in Class','status'=>'2');
          }else{
            $teacherData = array(
              'bid'=>$check['id'],
              'uid'=>$addStaff['id'],
              'class_id'=>$data->checkon->classId,
              'update_date'=>time(),
              'date_time'=>time()
            );
            $data=$this->db->insert('class_teacher',$teacherData);
            $sendRes = array('msg'=>'Teacher Added','status'=>'1');
          }
          echo $response= json_encode(array('checkon'=>$sendRes));
        }else{
          $sendRes = array('msg'=>'No Data Found','status'=>'0');
  ...
  function addStudentToClass(){
    ...
    if(key($data)=="checkon"){
      ...
      if(!empty($check['id'])){
        $bid = $check['id'];
        $getTeacherById = $this->app->getTeacherById($check['id']);
        if(!empty($getTeacherById['id'])){
            $bid = $getTeacherById['bid'];
        }
        $uploadedImg='upload/nextpng.png';
        if(!empty($data->checkon->image)){
            $img=base64_decode($data->checkon->image);
            $imgs = imagecreatefromstring($img);
        if($imgs != false){
        ...
        $newMobile = "55".rand(1000,9999).$nmid;
        $saveData=array(
            'firebassid'=>"",
            'otp'=>"",
            'm_id'=>$temp_.$nmid,
        ...
        if($id){
            ...
            $omid = $this->app->getMaxRollNo()['student_code'];
            $temp_ = "STU";
            if($omid == ''){
                $student_code = $temp_.'0001';
            }else{
                $str1 = substr($omid,3);
                $str1 = $str1 + 1;
                $str2 = str_pad($str1 , 4 , 0 , STR_PAD_LEFT);
                $student_code = $temp_.$str2;
            }
            $studentData=array(
                'bid'=>$bid,
                'uid'=>$id,
                'class_id'=>$data->checkon->classId,
                'roll_no'=>$data->checkon->rollNo,
                'student_code'=>$student_code,
                'gender'=>$data->checkon->gender,
                'blood_group'=>$data->checkon->bloodGroup,
                'parent_name'=>$data->checkon->parentName,
                'parent_relation'=>$data->checkon->parentRelation,
                'parent_mobile'=>$data->checkon->parentMobile,
                'update_date'=>time(),
                'date_time'=>time()
            );
            $res = $this->app->addStudentToClass($studentData);
            echo $response= json_encode(array('checkon'=>array('msg'=>'Student Added Successfully','status'=>'1')));
        }else{
            $res=array('msg'=>'Failed to Add Student','status'=>'0');
            echo $response= json_encode(array('checkon'=>$res));
        }
  ...
  function updateStudentToClass(){
    ...
    if(key($data)=="checkon"){
      ...
      if(!empty($check['id'])){
          ...
          $this->db->where('uid',$data->checkon->sid);
          $this->db->where('bid',$bid);
          $res=$this->db->update('student_detail',$studentData);
          if($res){
            $studentData=array(
              'name'=>$data->checkon->name,
              'dob'=>$data->checkon->dob,
            ...
            if($data->checkon->imageChanged=="1"){
              $uploadedImg='upload/nextpng.png';
              $img=base64_decode($data->checkon->image);
              $imgs = imagecreatefromstring($img);
              if($imgs != false){
              ...
              $studentData['image']=$uploadedImg;
            }
            $this->db->where('id',$data->checkon->sid);
            $res=$this->db->update('login',$studentData);
            echo $response= json_encode(array('checkon'=>array('msg'=>'Student Updated Successfully','status'=>'1')));
          }else{
            $res=array('msg'=>'Failed to Update Student','status'=>'0');
            echo $response= json_encode(array('checkon'=>$res));
          }
      }
    }
  ...
  function getCompanyStudents(){
    ...
    if(key($data)=="checkon"){
      ...
      if(!empty($check['id']) && $check['user_group']==2){
        ...
        if(!empty($getTeacherById['id'])){
          $bid = $getTeacherById['bid'];
          $getCompanyClasses = $this->app->getCompanyClasses($bid,$data->checkon->offlineTime);
          $getCompanyTeachers = $this->app->getCompanyTeachers($bid,$data->checkon->offlineTime);
          $getCompanyStudents = $this->app->getCompanyStudents($bid,$data->checkon->offlineTime);
          $getCompanyStudentDetails = $this->app->getCompanyStudentDetails($bid,$data->checkon->offlineTime);
          $getCompanyStudentAttendance = $this->app->getCompanyStudentAttendance($bid,$data->checkon->offlineTime);
        }else{
          $getCompanyTeachers = array();
          $getCompanyClasses = array();
          $getCompanyStudentAttendance = array();
          $getCompanyStudents = array();
          $getCompanyStudentDetails = $this->app->getParentStudents($check['mobile']);
          foreach($getCompanyStudentDetails as $st){
            $getCompanyStudents[] = $this->app->userdetails($st->uid);
            $getCompanyClasses[] = $this->app->getStudentClassById($st->class_id);
            $getCompanyStudentAttendance = array_merge($getCompanyStudentAttendance,$this->app->getStudentAttendanceById($st->id,$data->checkon->offlineTime));
          }
        }
        foreach($getCompanyStudents as $key=> $student){
            $url=base_url();
            $user_image=$url.'/'.$student['image'];
            $im = file_get_contents($user_image);
            $user_image = base64_encode($im);
            $getCompanyStudents[$key]['image'] = $user_image;
        }
        
        echo $response= json_encode(array('classes'=>$getCompanyClasses,'classTeacher'=>$getCompanyTeachers,'students'=>$getCompanyStudents,'studentAttendance'=>$getCompanyStudentAttendance,'studentDetails'=>$getCompanyStudentDetails));
      }else if(!empty($check['id']) && $check['user_group']==1){
        if($check['validity']!="" && $check['validity']>time()){
      ...
      if(!empty($check['id']) && $check['user_group']==1 && $check['prime_att']==1){
        $getCompanyClasses = $this->app->getCompanyClasses($check['id'],$data->checkon->offlineTime);
        $getCompanyTeachers = $this->app->getCompanyTeachers($check['id'],$data->checkon->offlineTime);
        $getCompanyStudents = $this->app->getCompanyStudents($check['id'],$data->checkon->offlineTime);
        $getCompanyStudentDetails = $this->app->getCompanyStudentDetails($check['id'],$data->checkon->offlineTime);
        $getCompanyStudentAttendance = $this->app->getCompanyStudentAttendance($check['id'],$data->checkon->offlineTime);
        ...
        echo $response= json_encode(array('classes'=>$getCompanyClasses,'classTeacher'=>$getCompanyTeachers,'students'=>$getCompanyStudents,'studentAttendance'=>$getCompanyStudentAttendance,'studentDetails'=>$getCompanyStudentDetails));
          ...
          $res=array('msg'=>'Failed','prime'=>'0');
          echo $response= json_encode(array('checkon'=>$res,'prime'=>'0'));
      }
    }
  }
  
  function addStudentAttendanceByQr(){
    $data=json_decode(file_get_contents("php://input"));
    if(key($data)=="checkon"){
      $check=$this->app->checkMobile($data->checkon->mobile);
      if(!empty($check['id'])){
        ...
        $studentCode = explode('/', $data->checkon->rollNo);
        $student = $this->app->getStudentByRollNo($bid,end($studentCode));
        if(!empty($student['id'])){
          $start_time = strtotime(date("d-m-Y 00:00:00"));
          $end_time = strtotime(date("d-m-Y 23:59:59"));
          $getStudentAttendanceByTime = $this->app->getStudentAttendanceByTime($bid,$student['id'],$start_time,$end_time);

          if(!empty($getStudentAttendanceByTime) && ($getStudentAttendanceByTime['student_status']==1 || $getStudentAttendanceByTime['student_status']==0)){
              $studentData=array(
                'added_by'=>$check['id'],
                'student_status'=>$data->checkon->status,
                'time'=>time(),
                'update_date'=>time()
            );
            $this->db->where('id',$getStudentAttendanceByTime['id']);
            $res=$this->db->update('student_attendance',$studentData);
          ...
          if($res){
            $msg = "Present";
            if($data->checkon->status=="0"){
                $msg = "Absent";
            }else if($data->checkon->status=="2"){
                $msg = "Check In";
            }else if($data->checkon->status=="3"){
                $msg = "Check Out";
            }
            echo $response= json_encode(array('checkon'=>array('msg'=>$msg,'name'=>$student['name'],'image'=>$student['image'],'status'=>'1')));
          }else{
            $res=array('msg'=>'Failed','status'=>'0');
            echo $response= json_encode(array('checkon'=>$res));
          }
        }else{
          $res=array('msg'=>'Student Not Found','status'=>'0');
            echo $response= json_encode(array('checkon'=>$res));
        }
      }
    }
  ...
  function syncBioAttendance(){
    $data=json_decode(file_get_contents("php://input"));
    if(key($data)=="checkon"){
      $check=$this->app->checkMobile($data->checkon->mobile);
      if(!empty($check['id']) && $check['user_group']==1){
        $getBioDevice = $this->app->getBioDevice($check['id']);
        $Msg = "No Device Found";
        foreach($getBioDevice as $device){
          $ch = curl_init();
          $headers  = ['Content-Type: application/x-www-form-urlencoded'];
          $fromDate = date("Y-m-d",$check['date']);
          ...
          curl_setopt($ch, CURLOPT_URL,"http://103.30.72.117:7788/WebService.asmx/liveEmsTransaction");
          ...
          if($result->Status=="Success" && !empty($result->data)){
            foreach($result->data as $row){
              $getUserByBioId = $this->app->getUserByBioId($row->EnrollmentNo,$check['id']);
              if(isset($getUserByBioId)){
                $userCmp = $this->app->getUserCompany($getUserByBioId['id']);
                if( !empty($userCmp['business_id']) && $userCmp['business_id']==$check['id']){
                  $checkOffline = $this->app->checkIoTime($getUserByBioId['id'],$check['id'],strtotime($row->PunchDateTime));
                  if(empty($checkOffline)){
                    ...
                    $offline_at = $this->app->checkOfflineAt($getUserByBioId['id'],$check['id'],$start_time,$end_time);
                    $mode = "in";
                    if($userCmp['hostel']==1){
                        $mode = "out";
                    }
  ...
  function updateCompanyUserSalaryReport(){
    $data=json_decode(file_get_contents("php://input"));
    if(key($data)=="checkon"){
      $check=$this->app->checkMobile($data->checkon->mobile);
      if(!empty($check['id'])){
          ...
          $reportData=array(
            'bid'=>$data->checkon->bid,
            'uid'=>$data->checkon->uid,
            'start_date'=>$data->checkon->start_date,
            'end_date'=>$data->checkon->end_date,
            'days'=>$data->checkon->days,
            'present'=>$data->checkon->present,
            'absent'=>$data->checkon->absent,
            'half_day'=>$data->checkon->half_day,
            'week_off'=>$data->checkon->week_off,
            'holiday'=>$data->checkon->holiday,
            'leaves'=>$data->checkon->leaves,
            'ed'=>$data->checkon->ed,
            'short_leave'=>$data->checkon->short_leave,
            'net_payable'=>$data->checkon->net_payable
          );
          
           if($data->checkon->id=="0"){
             $res=$this->db->insert('salary_report',$reportData);
           }else{
             $this->db->where('id',$data->checkon->id);
             $this->db->where('bid',$check['id']);
             $res=$this->db->update('salary_report',$reportData);
           }
           
          if($res){
            echo $response= json_encode(array('checkon'=>array('msg'=>'Working Days Updated Successfully','status'=>'1')));
          }else{
            $res=array('msg'=>'Failed to Update Working Days','status'=>'0');
            echo $response= json_encode(array('checkon'=>$res));
          }
      }
    }
  ...
  function updateCompanyUserCtc(){
    ...
    if(key($data)=="checkon"){
      ...
      if(!empty($check['id'])){
          ...
          $reportData=array(
            'business_id'=>$data->checkon->business_id,
            'user_id'=>$data->checkon->user_id,
            'basic'=>$data->checkon->basic,
            'basic_value'=>$data->checkon->basic_value,
            'total_ctc_amount'=>$data->checkon->total_ctc_amount,
            'da'=>$data->checkon->da,
            'da_type'=>$data->checkon->da_type,
            'da_value'=>$data->checkon->da_value,
            'da_amount'=>$data->checkon->da_amount,
            'hra'=>$data->checkon->hra,
            'hra_type'=>$data->checkon->hra_type,
            'hra_value'=>$data->checkon->hra_value,
            'hra_amount'=>$data->checkon->hra_amount,
            'meal'=>$data->checkon->meal,
            'meal_type'=>$data->checkon->meal_type,
            'meal_value'=>$data->checkon->meal_value,
            'meal_amount'=>$data->checkon->meal_amount,
            'conveyance'=>$data->checkon->conveyance,
            'conveyance_type'=>$data->checkon->conveyance_type,
            'conveyance_value'=>$data->checkon->conveyance_value,
            'conveyance_amount'=>$data->checkon->conveyance_amount,
            'medical'=>$data->checkon->medical,
            'medical_type'=>$data->checkon->medical_type,
            'medical_value'=>$data->checkon->medical_value,
            'medical_amount'=>$data->checkon->medical_amount,
            'special'=>$data->checkon->special,
            'special_type'=>$data->checkon->special_type,
            'special_value'=>$data->checkon->special_value,
            'special_amount'=>$data->checkon->special_amount,
            'ta'=>$data->checkon->ta,
            'ta_type'=>$data->checkon->ta_type,
            'ta_value'=>$data->checkon->ta_value,
            'ta_amount'=>$data->checkon->ta_amount,
            'pf'=>$data->checkon->pf,
            'pf_type'=>$data->checkon->pf_type,
            'pf_value'=>$data->checkon->pf_value,
            'pf_amount'=>$data->checkon->pf_amount,
            'esi'=>$data->checkon->esi,
            'esi_type'=>$data->checkon->esi_type,
            'esi_value'=>$data->checkon->esi_value,
            'esi_amount'=>$data->checkon->esi_amount,
            'other'=>$data->checkon->other,
            'other_type'=>$data->checkon->other_type,
            'other_value'=>$data->checkon->other_value,
            'other_amount'=>$data->checkon->other_amount
          );
          
           if($data->checkon->id=="0"){
             $res=$this->db->insert('user_ctc',$reportData);
           }else{
             $this->db->where('id',$data->checkon->id);
             $this->db->where('business_id',$check['id']);
             $res=$this->db->update('user_ctc',$reportData);
           }
           
          if($res){
            echo $response= json_encode(array('checkon'=>array('msg'=>'Salary Updated Successfully','status'=>'1')));
          }else{
            $res=array('msg'=>'Failed to Update Salary','status'=>'0');
            echo $response= json_encode(array('checkon'=>$res));
          }
      }
    }
  ...
  function testNot(){
      //print_r(phpinfo());
    $notificationTitle = "Report of ".date("d-m-Y",strtotime("-1 day"));
    $notificationMessage = "Present: 0, Absent: 0, Late: 0";
    $this->sendNotification("cD2rBdqQT8WgOmxG4OuhY8:APA91bHlznOoqtxYMBzJsUinDD-1mrX3mmhQ7-QtSTK3I41ziVpphqRtpRajGPouwcLbL5mJontE-V294ITgDWGYDPe_aohCrZkFwZvINEstMk7km2nZKw896Qw5F_sogbDH1xlY7t3o",$notificationTitle,$notificationMessage);
  }
  
  function dailyReportNotification(){
    $cronConfig=$this->app->getCronConfig();
    $configData = array(
      'size'=>$cronConfig['size']+100
    );
    if($cronConfig['date']!=date("d-m-Y")){
        $allBusiness=$this->app->getAllBusiness($cronConfig['size']);
        if(count($allBusiness)!=100){
          $configData = array(
            'size'=>0,
            'date'=>date("d-m-Y")
          );
        }
        foreach($allBusiness as $bus){
          $usersData = $this->app->getCompanyUsers($bus->id);
          $start_time = strtotime(date("d-m-Y 00:00:00",strtotime("-1 day")));
    			$end_time = strtotime(date("d-m-Y 23:59:59",strtotime("-1 day")));
          $totalPresent = 0;
          $totalAbsent = 0;
          $totalLate = 0;
          $totalEarly = 0;
          $dayAt = $this->app->getCompanyAttendanceByDate($bus->id,$start_time,$end_time);
          if(!empty($usersData)){
            foreach($usersData as $user){
              ...
              $rules = $this->app->getRule($bus->id,$user->rule_id);
              $mispunch = "0";
              $ca_wo_lofi = "0";
              $mark_ab_week = "0";
              $ov_shift = "0";
              ...
              if(($user->doj!="" || $start_time>=$user->doj) && ($user->left_date=="" || $start_time<$user->left_date)){
                $uid =$user->user_id;
                $user_at = array_filter($dayAt, function($val) use($uid){
                  return ($val->user_id==$uid);
                });
                if(!empty($day_shift_start)){
                  $shift_start = $day_shift_start[date('N',$start_time)-1];
            ...
            $notificationTitle = "Report of ".date("d-m-Y",strtotime("-1 day"));
            $notificationMessage = "Present: ".$totalPresent.", Absent: ".$totalAbsent.", Late: ".$totalLate;
            $notificationData=array(
              'uid'=>$bus->id,
              'title'=>$notificationTitle,
              'message'=>$notificationMessage,
            ...
            $res=$this->db->insert('notifications',$notificationData);
            if(!empty($bus->firebassid)){
              $message = [
                'token' => $bus->firebassid,
                'notification' => [
                    'title' => $notificationTitle,
                    'body' => $notificationMessage,
                ],
        ...
        $this->db->where('id',1);
        $res=$this->db->update('cron_config',$configData);
    }
  }
  
  function addUserPayrollHistory(){
    $data=json_decode(file_get_contents("php://input"));
    if(key($data)=="checkon"){
      $check=$this->app->checkMobile($data->checkon->mobile);
      if(!empty($check['id'])){
        $reportData = array(
          'business_id' => $check['id'],
          'payroll_master_id'	=> $data->checkon->payrollMasterId,
          'user_id	' => $data->checkon->uid,
          'pay_date' => $data->checkon->payDate,
          'amount	' => $data->checkon->amount,
          'remarks' => $data->checkon->remarks,
          'status' => 1,
          'settled'	=>$data->checkon->settled,
          'paid' =>$data->checkon->paid,
          'payroll_id' =>"0",
          'date' => $data->checkon->payDate
        );
        $res=$this->db->insert('payroll_history',$reportData);
        if($res){
          echo $response= json_encode(array('checkon'=>array('msg'=>'Added Successfully','status'=>'1')));
        }else{
          $res=array('msg'=>'Failed to Add','status'=>'0');
          echo $response= json_encode(array('checkon'=>$res));
        }
      }
    }
  ...
  function settleUserAdvance(){
    $data=json_decode(file_get_contents("php://input"));
    if(key($data)=="checkon"){
      $check=$this->app->checkMobile($data->checkon->mobile);
      if(!empty($check['id'])){
        if(count($data->checkon->subSettle)>0){
            $this->db->query("update payroll_history set status=2 where payroll_id=".$data->checkon->subSettle[0]->payrollId);
            foreach($data->checkon->subSettle as $settle){
                $reportData = array(
                  'business_id' => $check['id'],
                  'payroll_master_id'	=> 2,
                  'user_id	' => $settle->uid,
                  'pay_date' => $settle->payDate,
                  'amount	' => $settle->amount,
                  'remarks' => "Deduction",
                  'status' => 1,
                  'settled'	=> 1,
                  'paid' => 1,
                  'payroll_id' =>$settle->payrollId,
                  'date' => $data->checkon->payDate
                );
                $res=$this->db->insert('payroll_history',$reportData);
            }
            ...
            if($res){
                $updateData = array('settled'=>1);
    			$this->db->where('id',$data->checkon->subSettle[0]->payrollId);
  ...
  function oldsendNotification($target,$title,$message){
    //API URL of FCM
    $url = 'https://fcm.googleapis.com/fcm/send';
    ...
    //$target = "c9d2nf5bTNCBmweBArZK0g:APA91bF4HH7J6qCwDXbP4_1lysE4VcyQQbsMp-BEoH0P7-HY12Y6kiKHKeodvZCHxLOWLvAaLWaZFDe6rSLaF1ubV-Dk60R0KolX_Qwd5FBCxL7NCygwW7l5xTfktGIvMdmHMvqPDHuf";
  ...
  public function recalculateSalary(){
    ...
    if(key($data)=="checkon"){
      $check=$this->app->checkMobile($data->checkon->mobile);
      if(!empty($check['id'])){
        $loginID = $check['id']; // THIS IS BUSINESS ID
        ...
        if(!empty($empList)){
          if(!empty($data->checkon->dateFrom)){
            $yearName  = date('Y', strtotime($data->checkon->dateFrom));
            $monthName = date('m', strtotime($data->checkon->dateFrom));
          }
          else{
            $yearName = date('Y');
            $monthName = date('m');
          ...
          $holidays = $this->web->getHoliday($loginID);
          $holiday_array = array();
          if($holidays){
            foreach($holidays as $holiday){
              $holiday_array[] = array(
          ...
          foreach ($empList as $key => $dataVal) {
            ...
            $getAmount = $this->app->getTypeTotalAmount($loginID,$dataVal->user_id, $yearName, $monthName, '1' );
            ...
            $getTotalDeduction = $this->app->getTypeTotalAmount($loginID,$dataVal->user_id, $yearName, $monthName, '2' );

            // HARA GET TOTAL PAID AMOUNT
            $getTotalPaid                = $this->app->getTypeTotalAmount($loginID,$dataVal->user_id, $yearName, $monthName, '3' );
            $empList[$key]->getTotalPaid =  $getTotalPaid['addAmount'];

            $empList[$key]->deductionAmount =  $getTotalDeduction['addAmount'];

            $groups = $this->web->getUserGroup($dataVal->business_group);
            $grp = array();
            $day_shift_start = array();
            $day_shift_end = array();
            ...
            $leaves = $this->web->getEmpLeaves($dataVal->user_id);
            $leaves_array = array();
            if($leaves){
              foreach($leaves as $leave){
                $from_date_leave=date_create(date("Y-m-d",$leave->from_date));
            ...
            $rules = $this->web->getRule($loginID,$dataVal->rule_id);
            $mispunch = "0";
            $ca_wo_lofi = "0";
            $mark_ab_week = "0";
            $ov_shift = "0";
        ...
        echo $response= json_encode(array('checkon'=>array('msg'=>'Recalcuated Successfully','status'=>'1')));
      }
    }
  }
  ...
  function addStudentParent(){
    ...
    if(key($data)=="checkon"){
      $check=$this->app->checkMobile($data->checkon->mobile);
      if(!empty($check['id']) && $check['user_group']==2){
        $addStaff=$this->app->checkMobile($data->checkon->studentMobile);
        if(!empty($addStaff)){
          if(substr($data->checkon->studentMobile,0,2)=="55"){
            $studentDetail = $this->app->getStudentDetail($addStaff['id']);
            if(isset($studentDetail) && $studentDetail['parent_mobile']==""){
              $studentData=array(
                'parent_mobile'=>$data->checkon->mobile,
                'update_date'=>time()
              );
              $this->db->where('id',$studentDetail['id']);
              $data= $this->db->update('student_detail',$studentData);
              $sendRes = array('msg'=>'Student Added Successfully','status'=>'1');
            }else{
              $sendRes = array('msg'=>'Unable to Add','status'=>'0');
            }
          }else{
            $studentDetail = $this->app->getStudentHostelDetail($addStaff['id']);
            if(isset($studentDetail) && $studentDetail['parent_mobile']==""){
              $studentData=array(
                'parent_mobile'=>$data->checkon->mobile
              );
              $this->db->where('id',$studentDetail['id']);
              $data= $this->db->update('hostel_detail',$studentData);
              $sendRes = array('msg'=>'Student Added Successfully','status'=>'1');
            }else{
              $sendRes = array('msg'=>'Unable to Add','status'=>'0');
            }
 ...
 function getCompanyDepartments(){
    ...
    if(key($data)=="checkon"){
      $check=$this->app->checkMobile($data->checkon->mobile);
      if(!empty($check['id']) && $check['user_group']==1){
        $result=$this->app->getCompanyDepartments($check['id']);
        echo $response= json_encode(array('checkon'=>$result));
      }
    }
  }
  ...
  function getCompanySubDepartments(){
    ...
    if(key($data)=="checkon"){
      $check=$this->app->checkMobile($data->checkon->mobile);
      if(!empty($check['id']) && $check['user_group']==1){
        $result=$this->app->getCompanySubDepartments($check['id'],$data->checkon->department);
        echo $response= json_encode(array('checkon'=>$result));
      }
    }
  }

  function getCompanyAppointByDep(){
    $data=json_decode(file_get_contents("php://input"));
    if(key($data)=="checkon"){
      $check=$this->app->checkMobile($data->checkon->mobile);
      if(!empty($check['id']) && $check['user_group']==1){
        $result=$this->app->getCompanyAppointByDep($check['id'],$data->checkon->department,$data->checkon->subdepart);
        if(!empty($result)){
          $result['shiftList'] = json_decode($result['shiftList']);
          $holidays = array();
           foreach(json_decode($result['holidays']) as $day){
             $holidays[] = array(
  ...
  function getCompanyAppointmentUpdate(){
    $data=json_decode(file_get_contents("php://input"));
    if(key($data)=="checkon"){
      $check=$this->app->checkMobile($data->checkon->mobile);
      if(!empty($check['id']) && $check['user_group']==1){

        $appointment['slot_diff'] = $data->checkon->slot_diff;
        $appointment['shiftList'] = json_encode($data->checkon->shiftList);
        $appointment['holidays'] = json_encode($data->checkon->holidays);

        $chk = $this->app->getCompanyAppointByDep($check['id'],$data->checkon->department,$data->checkon->subdepart);
        if(!empty($chk)){
          $response = $this->app->updateCmpAppointment($appointment,$chk['id']);
        }
        else{
          $response = $this->app->insertCmpAppointment($appointment);
        }
        $sendRes = array('msg'=>'Appoint Updated Successfully','status'=>'1');
        echo $response= json_encode(array('checkon'=>$sendRes));
      }
    }
  }
      ...
      if(!empty($check['id']) && $check['user_group']==2){
        ...
        if(!empty($userCmp)){
          $data = array(
            'bid'=>$userCmp['business_id'],
            'uid'=>$check['id'],
            'name'=>$data->checkon->client_name,
            'mobile'=>$data->checkon->client_mobile,
            'city'=>$data->checkon->client_city,
            'location'=>$data->checkon->client_location,
            'date_time'=>time()
          );
          $res = $this->app->addClient($data);
          if($res == 1){
            $sendRes = array('msg'=>'Client Added Successfully','status'=>'1');
          }else{
            $sendRes = array('msg'=>'Failed to Add Client','status'=>'0');
          }
          echo $response= json_encode(array('checkon'=>$sendRes));
        }
      }
  ...
  function addApiLog(){
    ...
    $log = array(
        'mobile'=>$data->checkon->mobile,
        'request'=>$data->checkon->request,
        'response'=>$data->checkon->response,
        'date_time'=>time()
    );
    //$res = $this->app->addApiLog($log);
  }
  
  private function sendNotification($message){
    // $serviceAccountPath = APPPATH.'../firebaseAdmin.json';
    // $projectId = 'checkon-b05a6';
