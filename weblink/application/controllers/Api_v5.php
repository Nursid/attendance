...
class Api_v5 extends CI_Controller {
 function __construct(){
       parent::__construct();
   $this->load->database();
   $this->load->model('Api_Model_v5','app');
...
function getCompanyUsersByStatus(){
 ...
 function updateEmpProfile(){
   ...
   if(key($data)=="checkon"){
     $check=$this->app->checkMobile($data->checkon->mobile);
     if(!empty($check['id']) && $check['user_group']==1){
      $doj = $this->app->updateDoj($data->checkon->uid,$check['id'],strtotime($data->checkon->doj),$data->checkon->rule_id,$data->checkon->qr,$data->checkon->gps,$data->checkon->face,$data->checkon->colleague,$data->checkon->autoGps,$data->checkon->gpsTracking);
 ...
 function addEmpLeave(){
  ...
  function addOfflineAttendance(){
      ...
      foreach($list as $at){
        ...
        if(!empty($check['id']) && $check['user_group']==2){
          ...
          if( !empty($userCmp['business_id']) && $userCmp['business_id']==$at->business_id){
            if($user_status['user_status']=="1"){
              $checkOffline = $this->app->checkOfflineAt($check['id'],$at->business_id,$at->time);
    ...
    //   }
      ...
      $sendRes = array('msg'=>$msg,'status'=>$status,'offline'=>$offline,'company_data'=>$cmp_array,'holiday'=>$holidays,'group'=>$groups,'leaves'=>$leaves,'logout'=>'0','sections'=>$sections,'user_section'=>$check['section'],'options'=>$options);
  ...
  function updateEmpOptions(){
   ...
   if(key($data)=="checkon"){
     ...
     if(!empty($check['id']) && $check['user_group']==1){
       ...
       if($dt){
         $res = $this->app->updateCmpOptions($check['id'],$data->checkon->auto_gps,$data->checkon->qr,$data->checkon->gps);
       }else{
         $options=array(
           'bid'=>$check['id'],
           'qr'=>$data->checkon->qr,
       ...
       $res = $this->app->updateEmpOptions($check['id'],$data->checkon->auto_gps,$data->checkon->qr,$data->checkon->gps);
       if($res){
    ...
    if(key($data)=="checkon"){
      ...
      if(!empty($check['id']) && $check['user_group']==2){
        ...
        if( !empty($userCmp['business_id'])){
            $user_status = $this->app->userCmpStatus($check['id'],$userCmp['business_id']);
          if($user_status['user_status']=="1"){
            $at_ver = $data->checkon->verified;
            $ver = $this->app->getEmpOptions($check['id'],$userCmp['business_id']);
            if($data->checkon->location!=""){
              if($ver['auto_gps']=="1"){
                $at_ver = "1";
              }
            }
 ...
 function addUserGPSFaceAttendance(){
    $data=json_decode(file_get_contents("php://input"));
    if(key($data)=="checkon"){
      $check=$this->app->checkMobile($data->checkon->mobile);
      if(!empty($check['id']) && $check['user_group']==2){
        ...
        if( !empty($userCmp['business_id'])){
          ...
          if($user_status['user_status']=="1"){
            ...
            //     }
                 ...
                 $selfieDataUrl = $data->checkon->selfie;
        $selfieFilename = "";
        $filePath = "";
       

        if (!empty($selfieDataUrl)) {
            // Extract base64 data from data URL
            if (preg_match('/^data:image\/(\w+);base64,/', $selfieDataUrl, $type)) {
                $selfieData = substr($selfieDataUrl, strpos($selfieDataUrl, ',') + 1);
                ...
                $fileName = time() . rand(1000, 9999) . '.' . $imageType;
                $filePath = "upload/selfie2/" . $fileName;
                 

                // Ensure upload directory exists
                if (!is_dir("upload/selfie2")) {
                    mkdir("upload/selfie2", 0777, true);
                }

                // Save file
                file_put_contents($filePath, $selfieData);
      ...
      if(!empty($check['id']) && $check['user_group']==2){
        ...
        if( !empty($userCmp['business_id'])){
          ...
          if($user_status['user_status']=="1"){
           // $at_ver = $data->checkon->verified;
          //  $ver = $this->app->getEmpOptions($check['id'],$userCmp['business_id']);
             $team = $this->app->getCompanyUsers($userCmp['business_id']);
            if(!empty($team)){
         $new_array = array();
