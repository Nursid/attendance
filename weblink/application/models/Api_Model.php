...
// 		$this->db->where('user_group','2');
	...
	public function getCompanyUsers($id){
		$sql = "SELECT user_request.user_id,(select name from login WHERE login.id = user_request.user_id) as name,(select image from login WHERE login.id = user_request.user_id) as image,(select business_group from login WHERE login.id = user_request.user_id) as business_group,user_request.user_status FROM `user_request` WHERE user_request.business_id='$id'";
	...
	public function getCompanyUsersByStatus($id,$status){
		$sql = "SELECT user_request.user_id,(select name from login WHERE login.id = user_request.user_id) as name,(select image from login WHERE login.id = user_request.user_id) as image,user_request.user_status FROM `user_request` WHERE user_request.business_id='$id' AND user_request.user_status='$status'";
		$res = $this->db->query($sql);
		return $res->result();
	}
