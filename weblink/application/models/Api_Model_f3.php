...
class Api_Model_f3 extends CI_Model
{
...
// 		$this->db->where('user_group','2');
	...
	public function getUserAttendanceByDate($start_time,$end_time,$uid,$bid,$verified){
		$sql = "SELECT * FROM `attendance` WHERE status=1 and io_time BETWEEN $start_time and $end_time and user_id='$uid' and bussiness_id='$bid' and manual!='2' and hostel=0 order by io_time DESC";
		$res = $this->db->query($sql);
		return $res->result();
	}
	...
	public function getCompanyUsersByBluetooth($id){
		$sql = "SELECT user_request.user_id,user_request.doj,user_request.left_date,user_request.rule_id,user_request.hostel,(select name from login WHERE login.id = user_request.user_id) as name,(select image from login WHERE login.id = user_request.user_id) as image,(select business_group from login WHERE login.id = user_request.user_id) as business_group,user_request.user_status,(select login.designation from login WHERE login.id = user_request.user_id) as designation,(select login.m_id from login WHERE login.id = user_request.user_id) as mid,(select login.bluetooth_ssid from login WHERE login.id = user_request.user_id) as bluetooth_ssid,(select login.bluetooth_mac from login WHERE login.id = user_request.user_id) as bluetooth_mac FROM `user_request` WHERE user_request.business_id='$id'";
		$res = $this->db->query($sql);
		return $res->result();
	}
