...
class Api_Model_v1 extends CI_Model
{
...
// 		$this->db->where('user_group','2');
	...
	public function getUserAttendance($id,$start,$end){
		$res = $this->db->query("SELECT * FROM attendance WHERE user_id='$id' AND status='1' AND verified='1' AND io_time >=$start AND io_time <$end order by id DESC");
		return $res->result();
	}
	...
	public function getCompanyUsers($id){
		$sql = "SELECT user_request.user_id,(select name from login WHERE login.id = user_request.user_id) as name,(select image from login WHERE login.id = user_request.user_id) as image,(select business_group from login WHERE login.id = user_request.user_id) as business_group,user_request.user_status,(select login.designation from login WHERE login.id = user_request.user_id) as designation,(select login.m_id from login WHERE login.id = user_request.user_id) as mid,(select login.doj from login WHERE login.id = user_request.user_id) as doj,(select login.emp_code from login WHERE login.id = user_request.user_id) as emp_code FROM `user_request` WHERE user_request.business_id='$id'";
		$res = $this->db->query($sql);
		return $res->result();
	}
	public function getUserAttendanceByDate($start_time,$end_time,$uid,$bid,$verified){
		$sql = "SELECT * FROM `attendance` WHERE status=1 and io_time BETWEEN $start_time and $end_time and user_id='$uid' and bussiness_id='$bid' and verified='$verified' order by io_time DESC";
		$res = $this->db->query($sql);
		return $res->result();
	}
	...
	public function getEmpProfile($id,$bid){
		$sql = "SELECT *,(select business_groups.name FROM business_groups WHERE business_groups.id = login.business_group) as business_group_name FROM `login` where id ='$id' and user_group ='2'";
		$res = $this->db->query($sql);
		return $res->row();
	}
	...
	public function updateEmpProfile($id,$name,$address,$email,$group,$designation,$dob,$gender,$doj,$education,$bluetoothSsid,$bluetoothMac,$empCode){
		$sql = "UPDATE login SET name ='$name',address='$address',email='$email',business_group='$group',designation='$designation',dob='$dob',gender='$gender',doj='$doj',education='$education',bluetooth_ssid='$bluetoothSsid',bluetooth_mac='$bluetoothMac',emp_code='$empCode' WHERE `login`.`id` = '$id'";
		$res = $this->db->query($sql);
		return $res;
	}
	public function getCompanyUserById($id,$empid){
		$sql = "SELECT user_request.user_id,(select name from login WHERE login.id = user_request.user_id) as name,(select image from login WHERE login.id = user_request.user_id) as image,(select business_group from login WHERE login.id = user_request.user_id) as business_group,user_request.user_status,(select login.designation from login WHERE login.id = user_request.user_id) as designation,(select login.m_id from login WHERE login.id = user_request.user_id) as mid,(select login.doj from login WHERE login.id = user_request.user_id) as doj,(select login.emp_code from login WHERE login.id = user_request.user_id) as emp_code FROM `user_request` WHERE user_request.business_id='$id' AND user_request.user_id='$empid'";
		$res = $this->db->query($sql);
		return $res->result();
	}
	...
	public function getLoginIdByQr($qr){
		$sql = "SELECT login_id,(SELECT login.user_group FROM login WHERE login.id=new_qr.login_id) as user_group FROM new_qr WHERE qr_code='$qr' and login_id IS NOT NULL";
		$res = $this->db->query($sql);
		return $res->row_array();
	}

	public function getCompanyUsersByBluetooth($id){
		$sql = "SELECT user_request.user_id,(select name from login WHERE login.id = user_request.user_id) as name,(select image from login WHERE login.id = user_request.user_id) as image,(select business_group from login WHERE login.id = user_request.user_id) as business_group,user_request.user_status,(select login.designation from login WHERE login.id = user_request.user_id) as designation,(select login.m_id from login WHERE login.id = user_request.user_id) as mid,(select login.doj from login WHERE login.id = user_request.user_id) as doj,(select login.bluetooth_ssid from login WHERE login.id = user_request.user_id) as bluetooth_ssid,(select login.bluetooth_mac from login WHERE login.id = user_request.user_id) as bluetooth_mac FROM `user_request` WHERE user_request.business_id='$id'";
		$res = $this->db->query($sql);
		return $res->result();
	}
	...
	public function requestPremium($id){
		$sql = "UPDATE login SET premium='1' WHERE id='$id'";
		$res = $this->db->query($sql);
		return $res;
	}
}
