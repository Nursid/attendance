...
// 		$this->db->where('user_group','2');
	...
	public function checkIoTime($id,$bid,$time, $io_timedubb){
		$sql = "SELECT * FROM attendance WHERE bussiness_id=$bid and user_id=$id and io_time BETWEEN $io_timedubb and $time and status=1 order by id DESC LIMIT 1";
		$res = $this->db->query($sql);
		return $res->row_array();
	}
	...
	*/
	...
	public function checkIoTime_old($id,$bid,$time){
		$sql = "SELECT * FROM attendance WHERE bussiness_id=$bid and user_id=$id and io_time='$time' and status=1 order by id DESC LIMIT 1";
		$res = $this->db->query($sql);
		return $res->row_array();
	}
	...
	public function getStudentAttendanceById($sid,$time){
		...
		public function checkIoTime_Stu($id,$bid,$time){
		$sql = "SELECT * FROM student_attendance  WHERE bid=$bid and student_id=$id and time='$time' and status=1 order by id DESC LIMIT 1";
		$res = $this->db->query($sql);
		return $res->row_array();
	}
