...
class Api_Model_v13 extends CI_Model
{
...
// 		$this->db->where('user_group','2');
	...
	public function getAppointmenthistory($loginid){
		  $sql="SELECT * FROM `book_appointment` where user_id='$loginid' ORDER BY slot";
		 $query=$this->db->query($sql);
		return $query->result();
	}
	public function getAppointmentbussiness($loginid,$startTime,$endTime){
		  $sql="SELECT * FROM book_appointment where 	bussiness_id='$loginid' and date BETWEEN $startTime and $endTime ORDER BY id DESC";
		 $query=$this->db->query($sql);
		return $query->result();
	}
	public function getbussnames($id){
		  $sql="SELECT id,name,mobile,image,address,user_group,image,mac,strength,m_id,firebassid FROM `login` where id='$id'";
		 $query=$this->db->query($sql);
		return $query->row_array();
	}
	...
	public function insertAttendance($data){
		...
		public function insertrfidAttendance($data){
		$res = $this->db->insert("student_checkin",$data);
		return $res;
	}
	...
	public function updateEmpProfile($id,$name,$image,$address,$email,$group,$designation,$dob,$gender,$doj,$education,$bluetoothSsid,$bluetoothMac,$empCode,$manager,$section,$department,$bioId){
		$sql = "UPDATE login SET name ='$name',image='$image',address='$address',email='$email',business_group='$group',designation='$designation',dob='$dob',gender='$gender',doj='$doj',education='$education',bluetooth_ssid='$bluetoothSsid',bluetooth_mac='$bluetoothMac',emp_code='$empCode',manager='$manager',section='$section',department='$department',bio_id='$bioId' WHERE `login`.`id` = '$id'";
		$res = $this->db->query($sql);
		return $res;
	}
	...
	public function getAttendanceByVerify($start_time,$end_time,$uid,$bid){
     ...
     public function verifyAttendance($id){
		$sql = "UPDATE attendance SET verified = '1',date=".time()." WHERE id = '$id'";
		$res = $this->db->query($sql);
		return $res;
	}
	...
	public function getUserAttendanceAll($id,$offlineTime){
		$res = $this->db->query("SELECT * FROM attendance WHERE user_id='$id' and date>=$offlineTime order by id DESC");
		return $res->result();
	}
	...
	public function getUserGpsTrackingAll($id,$offlineTime){
		$res = $this->db->query("SELECT * FROM gps_tracking WHERE uid='$id' AND status='1' and date_time>=$offlineTime order by id DESC");
		return $res->result();
	}
	...
	public function getUserGpsLocationAll($id,$offlineTime){
		$res = $this->db->query("SELECT * FROM gps_location WHERE uid='$id' AND status='1' and date_time>=$offlineTime order by id DESC");
		return $res->result();
	}
	...
	public function getMaxRollNo(){
		return $this->db->query("SELECT MAX(student_code) AS student_code FROM student_detail")->row_array();
	}
	...
	public function getCompanyStudents($bid,$time){
		$sql = "SELECT * FROM `login` WHERE company='$bid' and update_date>='$time'";
		$res = $this->db->query($sql);
		return $res->result();
	}

    public function getCompanyStudentDetails($bid,$time){
		$sql = "SELECT * FROM `student_detail` WHERE bid='$bid' and update_date>='$time'";
		$res = $this->db->query($sql);
		return $res->result();
	}
	...
	public function getStudentByRollNo($bid,$rollno){
		$sql = "SELECT * FROM `student_detail` WHERE bid='$bid' and student_code='$rollno' and status=1";
		$res = $this->db->query($sql);
		return $res->row_array();
	}
...
//	}
		...
		public function getBusinessByDeviceId($deviceId){
		$sql = "SELECT * FROM `Business_bioid` WHERE device_id='$deviceId'";
		$res = $this->db->query($sql);
		return $res->row_array();
	}
	...
	public function getParentStudents($mobile){
		$sql = "SELECT * FROM `student_detail` WHERE parent_mobile='$mobile'";
		$res = $this->db->query($sql);
		return $res->result();
	}
	...
	public function getCmpSalaryReport($id){
		$sql = "SELECT * FROM `salary_report` WHERE bid=$id";
		$res = $this->db->query($sql);
		return $res->result();
	}

	public function getCmpPayrollHistory($id){
		$sql = "SELECT * FROM `payroll_history` WHERE business_id=$id";
		$res = $this->db->query($sql);
		return $res->result();
	}
	
	public function getCmpUserCtc($id){
		$sql = "SELECT * FROM `user_ctc` WHERE business_id=$id";
		$res = $this->db->query($sql);
		return $res->result();
	}
	
	public function getCmpPayrollMaster(){
		$sql = "SELECT * FROM `payroll_master`";
		$res = $this->db->query($sql);
		return $res->result();
	}
	
	public function getStudentByRfid($uid,$bid){
	    $sql = "SELECT * FROM `login` WHERE rfid='$uid' and company='$bid'";
		$res = $this->db->query($sql);
		return $res->row_array();
	}

	public function checkOfflineStudentAt($id,$bid,$startTime,$endTime){
		$sql = "SELECT * FROM student_attendance WHERE bid=$bid and student_id=$id and time BETWEEN '$startTime' and '$endTime' and status=1 order by id DESC LIMIT 1";
		$res = $this->db->query($sql);
		return $res->row_array();
	}
	
	public function getNotifications($id){
		$sql = "SELECT * FROM `notifications` where uid='$id'";
		$res = $this->db->query($sql);
		return $res->result();
	}
	
	public function getAllBusiness($size){
		$sql = "SELECT * FROM `login` where user_group=1 order by id limit $size, 100";
		$res = $this->db->query($sql);
		return $res->result();
	}

	public function getCronConfig(){
	    $sql = "SELECT * FROM `cron_config` WHERE id='1'";
		$res = $this->db->query($sql);
		return $res->row_array();
	}
	
	public function getTypeTotalAmount($loginID,$user_id, $yearName, $monthName, $payRolType){
	...
	public function getStudentDetail($id){
		$sql = "SELECT * from student_detail where uid='$id' order by id DESC LIMIT 1";
		$res = $this->db->query($sql);
		return $res->row_array();
	}

	public function getStudentHostelDetail($id){
		$sql = "SELECT * from hostel_detail where uid='$id' order by id DESC LIMIT 1";
		$res = $this->db->query($sql);
		return $res->row_array();
	}
	...
	public function getCompanyDepartments($id){
		$sql = "SELECT *,(Select department.department from department WHERE assigned_department.department_id=department.id) as department_name FROM assigned_department where user_bussiness_id='$id'";
		$res = $this->db->query($sql);
		return $res->result();
	}

	public function getCompanySubDepartments($id,$dep){
		$sql = "SELECT *,(Select department_sub.depart_name from department_sub WHERE assigned_sdepartment.subdepart_id=department_sub.id) as subdepartment_name FROM assigned_sdepartment where user_business_id='$id' AND depart_id='$dep'";
		$res = $this->db->query($sql);
		return $res->result();
	}

	public function getCompanyAppointByDep($id,$dep,$subDep){
		$sql = "SELECT * FROM appoint_setting WHERE bussiness_id='$id' and department='$dep' and subdepart='$subDep'";
		$res = $this->db->query($sql);
		return $res->row_array();
	}

	public function insertCmpAppointment($data){
		$res = $this->db->insert("appoint_setting",$data);
		return $res;
	}

	public function updateCmpAppointment($data,$id){
		$this->db->where('id',$id);
		$res = $this->db->update("appoint_setting",$data);
		return $res;
	}
	
	public function getAppointmentbussinessByDate($loginid,$date){
		$sql="SELECT * FROM `book_appointment` where bookingdate='$date' and bussiness_id='$loginid' ORDER by CAST(book_appointment.slot as unsigned)";
	   $query=$this->db->query($sql);
	  return $query->result();
  	}
  	
  	public function getUserTaskById($id){
	...
	public function addClient($data){
		return $this->db->insert('client_list',$data);
	}

	public function getUserClients($bid,$uid){
		$sql = "SELECT * FROM `client_list` WHERE bid='$bid' AND uid='$uid'";
		$res = $this->db->query($sql);
		return $res->result();
	}
	
	public function getCmpClients($id){
		$sql = "SELECT * FROM `client_list` where bid='$id'";
		$res = $this->db->query($sql);
		return $res->result();
	}
	
	public function checkBookingTimeSlot($bid,$date,$time){
		$sql="SELECT * FROM `book_appointment` where bussiness_id='$bid' and bookingdate='$date' and booking_time='$time'";
	   $query=$this->db->query($sql);
	  return $query->result();
  	}
  	
  	public function addApiLog($data){
		return $this->db->insert('api_log',$data);
	}
