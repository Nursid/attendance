...
class Api_Model_v4 extends CI_Model
{
...
// 		$this->db->where('user_group','2');
	...
	public function getCompanyUsers($id){
		$sql = "SELECT user_request.user_id,user_request.doj,user_request.left_date,user_request.rule_id,user_request.month_weekly_off,(select name from login WHERE login.id = user_request.user_id) as name,(select image from login WHERE login.id = user_request.user_id) as image,(select business_group from login WHERE login.id = user_request.user_id) as business_group,user_request.user_status,(select login.designation from login WHERE login.id = user_request.user_id) as designation,(select login.m_id from login WHERE login.id = user_request.user_id) as mid,(select login.emp_code from login WHERE login.id = user_request.user_id) as emp_code,(select login.section from login WHERE login.id = user_request.user_id) as section,(select login.department from login WHERE login.id = user_request.user_id) as department FROM `user_request` WHERE user_request.business_id='$id' order by emp_code,doj";
		$res = $this->db->query($sql);
		return $res->result();
	}
	public function getUserAttendanceByDate($start_time,$end_time,$uid,$bid,$verified){
		$sql = "SELECT * FROM `attendance` WHERE status=1 and io_time BETWEEN $start_time and $end_time and user_id='$uid' and bussiness_id='$bid' order by io_time DESC";
		$res = $this->db->query($sql);
		return $res->result();
	}
	...
	public function getAttendanceByVerify($start_time,$end_time,$uid,$bid){
		$sql = "SELECT * FROM `attendance` WHERE status=1 and io_time BETWEEN $start_time and $end_time and user_id='$uid' and bussiness_id='$bid' and location='' order by id DESC";
		$res = $this->db->query($sql);
		return $res->result();
	}
	...
	public function checkOfflineAt($id,$bid,$time){
		$sql = "SELECT * FROM attendance WHERE bussiness_id=$bid and user_id=$id and io_time=$time";
		$res = $this->db->query($sql);
		return $res->result();
	}
	...
	public function updateEmpOptions($bid,$auto_gps){
		$sql = "UPDATE user_request SET auto_gps='$auto_gps' WHERE business_id='$bid'";
		$res = $this->db->query($sql);
		return $res;
	}
	...
	public function updateCmpOptions($bid,$auto_gps){
		$sql = "UPDATE business_at_option SET auto_gps='$auto_gps' WHERE bid='$bid'";
		$res = $this->db->query($sql);
		return $res;
	}
