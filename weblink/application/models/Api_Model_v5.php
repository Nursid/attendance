...
class Api_Model_v5 extends CI_Model
{
...
// 		$this->db->where('user_group','2');
	...
	public function getbussnames($id){
		  $sql="SELECT id,name,address,user_group,image,mac,strength,m_id FROM `login` where id='$id'";
		 $query=$this->db->query($sql);
		return $query->row_array();
	}
	...
	public function getCompanyUsers($id){
		$sql = "SELECT user_request.user_id,user_request.doj,user_request.left_date,user_request.rule_id,(select name from login WHERE login.id = user_request.user_id) as name,(select mobile from login WHERE login.id = user_request.user_id) as mobile,(select image from login WHERE login.id = user_request.user_id) as image,(select business_group from login WHERE login.id = user_request.user_id) as business_group,user_request.user_status,(select login.designation from login WHERE login.id = user_request.user_id) as designation,(select login.m_id from login WHERE login.id = user_request.user_id) as mid,(select login.emp_code from login WHERE login.id = user_request.user_id) as emp_code,(select login.section from login WHERE login.id = user_request.user_id) as section,(select login.department from login WHERE login.id = user_request.user_id) as department FROM `user_request` WHERE user_request.business_id='$id'";
	...
	public function getEmpProfile($id,$bid){
		$sql = "SELECT *,(select business_groups.name FROM business_groups WHERE business_groups.id = login.business_group) as business_group_name,(SELECT user_request.doj FROM user_request WHERE user_request.user_id='$id' and user_request.business_id='$bid') as user_doj,(SELECT user_request.qr FROM user_request WHERE user_request.user_id='$id' and user_request.business_id='$bid') as qr,(SELECT user_request.gps FROM user_request WHERE user_request.user_id='$id' and user_request.business_id='$bid') as gps,(SELECT user_request.face FROM user_request WHERE user_request.user_id='$id' and user_request.business_id='$bid') as face,(SELECT user_request.colleague FROM user_request WHERE user_request.user_id='$id' and user_request.business_id='$bid') as colleague,(SELECT user_request.auto_gps FROM user_request WHERE user_request.user_id='$id' and user_request.business_id='$bid') as auto_gps,(SELECT user_request.gps_tracking FROM user_request WHERE user_request.user_id='$id' and user_request.business_id='$bid') as gps_tracking,(SELECT user_request.rule_id FROM user_request WHERE user_request.user_id='$id' and user_request.business_id='$bid') as rule_id FROM `login` where id ='$id' and user_group ='2'";
		$res = $this->db->query($sql);
		return $res->row();
	}
	...
	public function updateBusinessGroup($id,$name,$startTime,$endTime,$weekOff,$dayStartTime,$dayEndTime){
		$sql = "UPDATE `business_groups` SET name ='$name',shift_start='$startTime',shift_end='$endTime',weekly_off='$weekOff',day_start_time='$dayStartTime',day_end_time='$dayEndTime' WHERE `business_groups`.`id` = '$id'";
		$res = $this->db->query($sql);
		return $res;
	}
	...
	public function updateEmpProfile($id,$name,$address,$email,$group,$designation,$dob,$gender,$doj,$education,$bluetoothSsid,$bluetoothMac,$empCode,$manager,$section,$department){
		$sql = "UPDATE login SET name ='$name',address='$address',email='$email',business_group='$group',designation='$designation',dob='$dob',gender='$gender',doj='$doj',education='$education',bluetooth_ssid='$bluetoothSsid',bluetooth_mac='$bluetoothMac',emp_code='$empCode',manager='$manager',section='$section',department='$department' WHERE `login`.`id` = '$id'";
		$res = $this->db->query($sql);
		return $res;
	}
	...
	public function updateDoj($id,$bid,$doj,$rule_id,$qr,$gps,$face,$colleague,$auto_gps,$gps_tracking){
		$sql = "UPDATE user_request SET doj='$doj',rule_id='$rule_id',qr='$qr',gps='$gps',face='$face',colleague='$colleague',auto_gps='$auto_gps',gps_tracking='$gps_tracking' WHERE user_id='$id' and business_id='$bid'";
		$res = $this->db->query($sql);
		return $res;
	}
	...
	public function checkOfflineAt($id,$bid,$time){
		$sql = "SELECT * FROM attendance WHERE bussiness_id=$bid and user_id=$id and io_time=$time and manual!='2'";
		$res = $this->db->query($sql);
		return $res->result();
	}

	public function getUserAttendanceAll($id){
		$res = $this->db->query("SELECT * FROM attendance WHERE user_id='$id' AND status='1' order by id DESC");
		return $res->result();
	}
	...
	public function updateCompanyMacByType($id,$name,$ssid,$mac,$strength,$type,$loc,$lat,$lon,$radius){
		$sql = "UPDATE sections SET ssid='$ssid', mac='$mac',strength='$strength',name='$name',location='$loc', latitude='$lat',longitude='$lon',radius='$radius' WHERE  bid='$id' AND type='$type'";
		$res = $this->db->query($sql);
		return $res;
	}
	...
	public function updateEmpOptions($bid,$auto_gps,$qr,$gps){
		$sql = "UPDATE user_request SET auto_gps='$auto_gps',qr='$qr',gps='$gps' WHERE business_id='$bid'";
		$res = $this->db->query($sql);
		return $res;
	}
	...
	public function updateCmpOptions($bid,$auto_gps,$qr,$gps){
		$sql = "UPDATE business_at_option SET auto_gps='$auto_gps',qr='$qr',gps='$gps' WHERE bid='$bid'";
		$res = $this->db->query($sql);
		return $res;
	}
