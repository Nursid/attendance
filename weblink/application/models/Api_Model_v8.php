...
class Api_Model_v8 extends CI_Model
{
...
// 		$this->db->where('user_group','2');
	...
	public function getCompanyUsers($id){
		$sql = "SELECT user_request.user_id,user_request.doj,user_request.left_date,user_request.rule_id,(select name from login WHERE login.id = user_request.user_id) as name,(select image from login WHERE login.id = user_request.user_id) as image,(select business_group from login WHERE login.id = user_request.user_id) as business_group,user_request.user_status,(select login.designation from login WHERE login.id = user_request.user_id) as designation,(select login.m_id from login WHERE login.id = user_request.user_id) as mid,(select login.emp_code from login WHERE login.id = user_request.user_id) as emp_code,(select login.bio_id from login WHERE login.id = user_request.user_id) as bio_id,(select login.section from login WHERE login.id = user_request.user_id) as section,(select login.department from login WHERE login.id = user_request.user_id) as department FROM `user_request` WHERE user_request.business_id='$id'";
	...
	public function updateCmpOptions($bid,$auto_gps,$qr,$gps,$field,$security,$face,$gpsSelfie,$fieldSelfie){
		$sql = "UPDATE business_at_option SET auto_gps='$auto_gps',qr='$qr',gps='$gps',field_duty='$field',four_layer_security='$security',face='$face',selfie_with_gps='$gpsSelfie',selfie_with_field_duty='$fieldSelfie' WHERE bid='$bid'";
		$res = $this->db->query($sql);
		return $res;
	}
	...
	public function getdevicebyid($bio){
		...
		public function getCompanyUsersbyempcode($Empcode,$id){
		$sql="SELECT * FROM `login` WHERE emp_code='$Empcode' AND company='$id'";
	    $query=$this->db->query($sql);
		return $query->row_array();
	}
