...
class Api_Model_v9 extends CI_Model
{
...
// 		$this->db->where('user_group','2');
	...
	public function getCompanyUsers($id){
		$sql = "SELECT user_request.user_id,user_request.doj,user_request.left_date,user_request.rule_id,user_request.hostel,(select name from login WHERE login.id = user_request.user_id) as name,(select image from login WHERE login.id = user_request.user_id) as image,(select business_group from login WHERE login.id = user_request.user_id) as business_group,user_request.user_status,(select login.designation from login WHERE login.id = user_request.user_id) as designation,(select login.m_id from login WHERE login.id = user_request.user_id) as mid,(select login.emp_code from login WHERE login.id = user_request.user_id) as emp_code,(select login.section from login WHERE login.id = user_request.user_id) as section,(select login.department from login WHERE login.id = user_request.user_id) as department,(select login.bio_id from login WHERE login.id = user_request.user_id) as bio_id FROM `user_request` WHERE user_request.business_id='$id'";
	...
	public function updateDoj($id,$bid,$doj,$rule_id,$qr,$gps,$face,$colleague,$auto_gps,$gps_tracking,$field,$security,$gpsSelfie,$fieldSelfie){
		$sql = "UPDATE user_request SET doj='$doj',rule_id='$rule_id',qr='$qr',gps='$gps',face='$face',colleague='$colleague',auto_gps='$auto_gps',gps_tracking='$gps_tracking',field_duty='$field',four_layer_security='$security',selfie_with_gps='$gpsSelfie',selfie_with_field_duty='$fieldSelfie' WHERE user_id='$id' and business_id='$bid'";
		$res = $this->db->query($sql);
		return $res;
	}
	...
	public function updateEmpOptions($bid,$auto_gps,$qr,$gps,$field,$security,$face,$gpsSelfie,$fieldSelfie,$gpsTracking){
		$sql = "UPDATE user_request SET auto_gps='$auto_gps',qr='$qr',gps='$gps',field_duty='$field',four_layer_security='$security',face='$face',selfie_with_gps='$gpsSelfie',selfie_with_field_duty='$fieldSelfie',gps_tracking='$gpsTracking' WHERE business_id='$bid'";
		$res = $this->db->query($sql);
		return $res;
	}
	...
	public function updateCmpOptions($bid,$auto_gps,$qr,$gps,$field,$security,$face,$gpsSelfie,$fieldSelfie,$gpsTracking){
		$sql = "UPDATE business_at_option SET auto_gps='$auto_gps',qr='$qr',gps='$gps',field_duty='$field',four_layer_security='$security',face='$face',selfie_with_gps='$gpsSelfie',selfie_with_field_duty='$fieldSelfie',gps_tracking='$gpsTracking' WHERE bid='$bid'";
		$res = $this->db->query($sql);
		return $res;
	}
	...
	public function getCompanyAttendanceByDate($bid,$start_time,$end_time){
		$res = $this->db->query("SELECT * FROM attendance WHERE bussiness_id='$bid' and io_time BETWEEN $start_time and $end_time and status='1'");
		return $res->result();
	}
