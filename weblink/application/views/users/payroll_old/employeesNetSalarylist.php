                    ...
                    <tbody>
                      ...
                      if (!empty($salEmpList)) {
                        ...
                        foreach ($salEmpList as $key => $empData) { 
                          ...
                          $salary_month = $this->db->select('DATE_FORMAT(date, "%Y-%m") as added_date')->from('salary')->where(['uid'=>$empData->emp_id])->get()->result_array();
                          if($salary_month){
                          ...
                          $tds = $this->db->select('salary_basic.amount,salary_basic.header_type,salary_basic.header_value')
                            ...
                            $advance = $this->db->select_sum('payroll_history.amount')
                                              ->from('payroll_history')
                                              ->join('payroll_master','payroll_master.id=payroll_history.payroll_master_id')
                                              ->where_in('payroll_master_id',[2,4,9])
                                              ->where("user_id",$empData->emp_id)
                                              ->where("DATE_FORMAT(pay_date,'%Y-%m')", $month)
                                              ->get()
                                              ->row();
                            ...
                            $total_tds = isset($tds->amount) ?  ($tds->header_type=="Manual" ? $tds->amount : round((((($salary->basic_value)/$number_of_days)*$empData->nwd)*$tds->header_value))/100) : 0;
